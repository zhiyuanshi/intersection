...
the same name is confusing.} The keys aspects are the notion of disjoint
intersections.
...
But a function type and an intersection
...
is ultimately the reason for incoherence. There are two important types of
overlap:
...
intersection are disjoint it is impossible that both of the preconditions of the
left decompositions are satisfied at the same time. More formally, with disjoint
intersections, we have the following theorem:
