...
Recently, Castagna et al.~\cite{Castagna:2014} studied an very interesting
...
The seminal paper by Schärli et al. introduced the ideas behind
...
Scala also has a notion of ``traits''. However, unlike what its name suggests,
...
\emph{implicit resolution of conflicts}. The traits modelled in \name are
intended to model the original trait idea closely, and conflicts must
be resolved explicitly. Schärli et al. document well the trade-offs
...
%  But our current language does not support this feature.
   ...
   \item Dynamic instantiation is supported in \name, but not in
     ...
     requires that all classes or traits are statically know. 
