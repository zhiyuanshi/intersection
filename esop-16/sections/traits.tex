...
\bruno{Something needs to be said about multiple inheritance here:
we can think of traits as giving us many of the benefits of multiple
inheritance, but without many of the problems.}
...
\bruno{The place for related work discussions is in related work, not here.}
...
\bruno{For the paper I don't like the ``in'' syntax. I think ``;''
  looks much better.}
...
them for type annotations of the self reference. \george{Note provided vs.
required methods in traits.}
...
\bruno{Our traits have constructors; regular traits do not.}
...
\paragraph{Traits with Dependencies.} \bruno{write a topic sentence: what is this
paragraph about?} The following example shows that a trait can depend on another
trait. The norm of a point is a generalized notion of distance of that point to
the origin. In the following example, we provide two definitions of norm via two
...
\george{Simpler example should do}
\begin{lstlisting}
...
\bruno{Don't just briefly mention the advantage! Actually show an example which
illustrates something, which is impossible say in Scala.}

Actually, since traits are just terms, traits can be defined inside a function,
passed around or returned just as normal terms. \george{Example: A function that
takes a trait with unknown implementation and instantiate it. But need to think
what the ``type'' of the trait should be in this case.} But one difference with
...
\bruno{Mention that the idea of trait translation is to follow the
  functional semantics of mixins by Cook.}
...
\bruno{There's no need to mention polymorphism at all, since
  desugaring could simply inline the definition of new.}
