...
($A * B$)\footnote{The notation $A * B$ is inspired by the
...
\bruno{type-safety and the final coherence theorem are not in the Coq file.}
...
paper for free\bruno{Do we compare enough with the traits paper to claim this?}.
...
\item \textbf{Implementation:} An implementation of an
  ...
  paper, are publicly available online\footnote{
    \url{https://github.com/zhiyuanshi/intersection}\bruno{That's not the correct url, is it? We need the implementation of fcore plus examples?}}.

\end{itemize*}

\bruno{Records and record operations missing in the appendix.}
\bruno{Proof of type-safety missing from appendix.}
