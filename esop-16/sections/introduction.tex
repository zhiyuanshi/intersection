...
insufficient for studying extensibility.
\end{comment}
...
Moreover, Dunfield illustrated that having a coherent semantics is highly
nontrivial.
...
\emph{merge operator} whose semantics \name is both type-safe and
coherent.
...
\footnote{The notation $A * B$ is inspired by the
  \emph{separating conjunction} construct in Reynolds' separation
...
disjointness. We have formalized the system and the proofs in Coq.

\bruno{A paragraph is missing here about traits.}
...
\item \textbf{Trait:} As an application of this work, we show how to build a
trait-based object-oriented language by leveraging the type system of \name. The
resulting source language enjoys many features described in the original trait
paper for free.

\item {\bf Formalization of \name and Proof of Coherence:} An
   elaboration semantics of \name into $\lambda$-calculus is
   given. Type-soundness and coherence are proved and formalized using
   the Coq theorem prover.

\item \textbf{Implementation:} An implementation of an
  extension of \name, as well as the examples presented in the
  paper, are publicly available\footnote{Examples are available on:
    \url{https://github.com/zhiyuanshi/intersection}}.
