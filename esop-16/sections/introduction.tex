...
allow only disjoint intersection types, he did not pursue this approach
...
mechanized the interesting metatheoretical results using the Coq proof
assistant, while the proof of type-safety and the coherence theorem can be found
in the appendix.

As an application of \name, we show that \name can serve as a
foundation of for a trait-based~\cite{scharli2003traits} object-oriented
...
dynamically, which cannot be achieved in many existing object-oriented
languages.
...
online\footnote{
\url{https://github.com/hkuplg/fcore/tree/develop/examples/traits}\bruno{What
we need here is a link to fcore! Otherwise, how do we run the examples?}}.
