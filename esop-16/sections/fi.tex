...
\begin{figure}[t]
  ...
  \caption{\name syntax.}
  \label{fig:fi-syntax}
\end{figure}\bruno{put figures on top.}
...
\begin{figure}
  ...
  \begin{mathpar}
    \formwf \\
    \rulewfvar \and \rulewffun \and \rulewfinter
  \end{mathpar}
...
\bruno{The underscores in the names of the rules look quite ugly. Why
  not drop them?}
...
rules for variables \reflabel{\labeltvar} and lambda abstractions
\reflabel{\labeltlam}.
...
in the appendix.\bruno{Does the target language does need unit? Any
  rules generating units? I don't think so, so it can be dropped.}
...
In rules \reflabel{\labelsubvar}, coercions are just
