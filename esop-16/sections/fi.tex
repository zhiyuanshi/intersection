...
and a merge operator.  This calculus is similar to Dunfield's
calculus~\cite{dunfield2014elaborating}.  The minor differences are
that, in Dunfield's language, the coercions are source language terms
and that function application allows the expression denoting the
function to have an intersection type. In contrast, in \name coercions
are target language terms and function applications do not allow the
expression denoting the applied function to have an intersection type.
