...
\bruno{TODO: Show example illustrating the issue with function application.
It may be worthwhile to discuss the issue with function
  ...
  problems that arise from having a more ``clever'' function
  application.}
\bruno{TODO: There are missing parts of the formalization regarding
  integers
(for example, subtyping, and typing). Formalization needs to be
...
denoting the applied function to have an intersection type.

In Section~\ref{sec:disjoint} we will show the necessary changes for supporting
...
\begin{figure}
  ...
  \label{fig:fi-type}
\end{figure}\bruno{Aren't we missing the uses of well-formedness
  rules. They are not being used anywhere at the moment! We are also
  missing the typing rules for integers and variables; as well as the
  subtyping rule for integers}
...
\paragraph{Typing Applications.} The more important difference bettwen
\name and Dunfield calculus is in the treatment of the
application rule. In Dunfield's calculus, if \lstinline$f$ and \lstinline$g$ are both
functions expecting an integer, the following program typechecks:
...
$\code{f : Int} \to \code{Bool}$ and $\code{g : Int} \to \code{Char}$, we could select $\code{f}$
from the intersection as follows:

\begin{lstlisting}
((\(h : Int (*$\to$*) Bool) (*$\to$*) h) (f ,, g)) 1
\end{lstlisting}
...
\noindent where $\code{idInt : Int} \to \code{Int}$ and $\code{idChar : Char} \to \code{Char}$ are,
...
are disjoint. Yet under naïve rules for intersection types, there are
two possible outcomes for this program :
\lstinline$1$ or \lstinline$'c'$. Thus application requires a
different treatment from Dunfield's, if we wish to have coherence.\bruno{work in progress}
...
\paragraph{Target Language.}\bruno{misses integers}
