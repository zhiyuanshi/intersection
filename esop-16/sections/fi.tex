...
The minor differences are that, in Dunfield's language, the coercions are source
...
denoting the applied function to have an intersection type. For example, In
Dunfield's calculus, if \lstinline$f$ and \lstinline$g$ are both functions
expecting an integer, the following program typechecks:
...
function by passing \lstinline$f ,, g$ to a function.
