...
for various programming language features.

\bruno{Our motivation for Dundfield work: build a language + address
extensibility issues.}

Unfortunatelly, while Dunfield calculus is type-safe, it lacks the important
property of \emph{coherence}: different derivations for the same
expression can lead to different results. The lack of coherence is
important disavantage for adoption of his core calculus in
implementations of programming languages, as the semantics of the
programming language becomes implementation dependent.
\bruno{Also lacks polymorphism, which is important for extensibility.}
...
\item {\bf Disjoint Intersection Types:} A new form of intersection
  ...
  super type) are allowed. An algorithmic specification for
