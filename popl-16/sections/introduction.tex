...
Previous work by Dunfield~\cite{} has shown the power of intersection
types and a merge operator. The presence of a merge operator adds
significant expressiveness to the language, allowing encodings for
many other language constructs as syntactic sugar. For example
single-field records are easily encoded as types with a label, and
multi-field records are encoded as the concatenation of single-field
records. Concatenation of records is expressed using intersection
types at the type-level and the corresponding merge operator and the
term level. Dunfield formalized a simply typed lambda calculus with
intersection types and the merge operator, and showed how to give a
...
translation is simple and elegant, and was shown to be type-safe.
...
been a wide interest in presenting solutions to the \emph{expression
  problem} in various communities. Presently there are several
solutions in the literature of functional
programming,\bruno{references} object-oriented programming and the
theorem proving communities. More recently, intersection types and
encoding of a merge operator in Scala have been shown to be useful to
solve additional challenges related to extensibility~\cite{}. Despite
the use of a wide-range of programming languages or theorem provers,
many solutions follow the similar ideas and are closely related to
type-theoretic encodings of datatypes. Clearly this seems to indicate
that a more foundational approach is lacking ... This is precisely
where the work of Dunfield is relevant: is work can be a starting 
point for a foundational calculus that can capture the key ideas 
behind the various solutions. 

Unfortunatelly, Dunfield calculus is still insufficient for
extensibility in two accounts.  Firstly it does not support parametric
polymorhism. More importantly while Dunfield calculus is type-safe, it
lacks the property of \emph{coherence}: different derivations for the
same expression can lead to different results. The lack of coherence
is important disavantage for adoption of his core calculus in
implementations of programming languages, as the semantics of the
programming language becomes implementation dependent.
...
Moreover we show that this approach extends to systems with parametric
...
\item {\bf Disjoint Intersection Types:} A new form of intersection
  type where only disjoint types (types that do not share a common
  super type) are allowed. A sound and complete algorithmic specification for
  determining whether two types are disjoint is presented.
  
\item {\bf Disjoint Quantification:}: A novel form of universal
quantification where type variables can have disjointness
constraints.

\item {\bf Coherence in the Presence of Merge:}  

\item {\bf Formalization of System \name:} An elaboration semantics of
  System \name into System F is given. Type-soundness and coherence are proved.
