...
This section presents the syntax, subtyping, typing, as well as the semantics of
\name.
...
type-level and the ``merges'' $e_1 \mergeop e_2$ at the term level. Below are
...
\paragraph{Contexts} Typing contexts $ \Gamma $ track bound type variables and
variables with their type $A$. We use $\subst {A} \alpha {B}$ to denote the
capture-avoiding substitution of $A$ for $\alpha$ inside $B$ and $\ftv \cdot$
for sets of free variables.

To focus on the key features that make this language interesting, we do not
include other forms such as type constants such as $\tyint$ and $\tystring$, and
fixpoints here, although they can be included in the formalization in standard
ways.
...
Figure~\ref{fig:fi-type}. At the moment, the reader are advised to omit the
...
type and covariant in its return type. A universal quantifier ($\forall$) is
covariant in its body. The three rules dealing with intersection types are just
what one would expect when interpreting types as sets. Under this
interpretation, for example, the rule \reflabel{\labelsubinter} says that if
$A_1$ is both the subset of $A_2$ and the subset of $A_3$, then $A_1$ is also
...
\george{Well-formedness still needs to be shown.}

The typing rules are shown in the bottom part of Figure~\ref{fig:fi-type}.
Again, the reader are advised to omit the gray-shaded part here, as they will be
explained later. The typing judgement is of the form: \[ \jtype \Gamma e A \] It
reads: ``in the typing context $\Gamma$, the term $e$ is of type $A$'' The rules
that are the same as in System $F$ are rules for variables
\reflabel{\labeltvar}, lambda abstractions \reflabel{\labeltlam}, and type
applications \reflabel{\labelttapp}. For the ease of discussion, in
\reflabel{\labeltblam}, we require the type variable introduced by the
quantifier is fresh. For programs with type variable shadowing, this requirement
can be met straighforwardly by variable renaming. The rule \reflabel{\labeltapp}
needs special attention as we add a subtyping requirement: the type of the
argument ($A_3$) is a subtype of that of the parameter ($A_1$).
\reflabel{\labeltmerge} means for merges $e_1 \mergeop e_2$, we typecheck $e_1$
and $e_2$ separately, and give it the intersection of the resulting types.

\subsection{Semantics}

We define the dynamic semantics of the call-by-value \name by means of a
type-directed translation to an extension of System $F$~\footnote{ For
simplicity, we will just refer to this system as ``System $F$'' from now on.}.
This translation turns merges into usual pairs, similar to Dunfield's
elaboration approach~\cite{dunfield2014elaborating}. In the end, the resulted
terms, highlighted in gray, can be typed and interpreted within System $F$. We
also tacitly assume the variables introduced in the gray part are generated from
a unique name supply and are always fresh. Below are the details.

\paragraph{Informal discussion}

\george{Wonder if still need this informal stuff.}

This subsection presents the translation informally. The idea is turning merges
into pairs. For example, \[ 1 \mergeop \code{"one"} \] becomes \pair 1
...
\tystring x)} {(1 \mergeop \code{"one"})} \] It will be translated to \[ \app
{(\lam x \tystring x)} {(\app {(\lamty x {\pair \tyint \tystring} {\proj 2 x})}
...
\]
generates a coercion function from $\tyint \inter \tybool$ to $\tybool$.
...
\reflabel{\labelsubinter} elaborate with intersection types.
...
type in the conclusions. Note that the two rules are syntatically the same and
