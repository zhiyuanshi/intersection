\section{The \namenaive Calculus} \rulelabel{sec:finaive}
...
\begin{figure*}
  ...
  \begin{mathpar}
    \formt \\
    \ruletvar \and \ruletlam \and \ruletapp \and \ruletblam \and \rulettapp
    \ruletmerge
  \end{mathpar}
...
al~\cite{frisch2008semantic}). The rule \rulelabel{Sub\_Fun} says that a
function is contravariant in its parameter type and covariant in its return
type. A universal quantifier ($\forall$) is covariant in its body. The three
rules dealing with intersection types are just what one would expect when
interpreting types as sets. Under this interpretation, for example, the rule
\rulelabel{Sub\_Inter} says that if $A_1$ is both the subset of $A_2$ and the
subset of $A_3$, then $A_1$ is also the subset of the intersection of $A_2$ and
$A_3$. In order to achieve coherence, \rulelabel{Sub\_Inter1} and
\rulelabel{Sub\_Inter2} additionally require the type on the right-hand side is
atomic.
...
% \end{enumerate}
% The first point is captured by two rules $ \rulelabelsubinterl $ and
% $ \rulelabelsubinterr $, whereas the second point by $ \rulelabelsubinter $.
...
\begin{lemma}[Subtyping is reflexive] \rulelabel{sub-refl}
Given a type $ A $, $ A \subtype A $.
\end{lemma}

\begin{lemma}[Subtyping is transitive] \rulelabel{sub-trans}
...
auxiliary judgments. The main typing judgment is of the form: $ \jtype \Gamma
...
\begin{figure}[h]
  ...
  \begin{array}{llrl}
    ...
    \text{Terms}    & E & \Coloneqq & x \mid \lam {(x \oftype T)} E \mid \app {E_1} {E_2} \mid \blam \alpha E \\
                    &   & \mid      & \tapp E T \mid \pair {E_1} {E_2} \mid \proj k E \\
    \text{Contexts} & G & \Coloneqq & \epsilon \mid G, \alpha \mid G, x \oftype T \\
  \end{array}
  \]
  \caption{Target language syntax.}
  \rulelabel{fig:f-syntax}
\end{figure}

% \bruno{Why is this lemma placed here?}
% \bruno{Generaly Speaking this text seems out of place.Move to 5.4, maybe?}

% The main translation judgment is $ \jtype \Gamma e A \yields E $ which
...
\begin{figure}[h]
  ...
  \rulelabel{fig:type-and-context-translation}
\end{figure}
...
\begin{restatable}[Subtyping rules produce type-correct coercion]{lemma}{lemmasub}
  \rulelabel{lemma:sub}
  If $ A_1 \subtype A_2 \yields C $, then $ \jtype \epsilon C {\im {A_1} \to \im {A_2}} $.
\end{restatable}
...
$\jtype \Gamma e A \yields E$ extends the typing judgment with an elaborated
...
\begin{restatable}[Translation preserves well-typing]{theorem}{theorempreservation}
  \rulelabel{theorem:preservation}
