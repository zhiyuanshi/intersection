...
and application of terms to types $\tapp e A$.  $e_1 \mergeop e_2$ is the
\emph{merge} of two terms $e_1$ and $e_2$. It can be used as either $ e_1 $ or $
e_2 $. If one regards $e_1$ and $e_2$ as objects, their merge will respond to
every method that one or both of them have. Merges of terms correspond to
intersections of types $A \inter B$.
...
not include other forms such as type constants such as $\tyint$ and $\tystring$,
and fixpoints here, although they can be included in the formalization in
standard ways and we are using them in discussions.
...
Figure~\ref{fig:fi-type}. At the moment, the reader is advised to omit the
...
type and covariant in its return type. \reflabel{\labelsubforall} a universal
