...
subtyping seems to have important advantages over syntactic subtyping. Recently,
...
$\code{Int}$ as $\alpha \setminus \code{Int}$, which is a shorthand for $\alpha
\wedge \neg \code{Int}$. As a comparison, such a type will need a disjoint
quantifier, like $\fordis \alpha {\code{Int}} \alpha$, in our system.
Unfortunately their calculus does not include a merge operator like ours. One
worthy avenue for future work is to study languages with a intersection types
