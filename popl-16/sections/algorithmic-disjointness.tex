...
\paragraph{Main judgement} The judgement $\isdisjoint \Gamma A B$ says two types
$A$ and $B$ are disjoint in a context $\Gamma$. The \rulelabel{DisjointInter1}
and \rulelabel{DisjointInter2} inductively distribute the relation itself over
the intersection constructor ($\inter$). We have derived \rulelabel{DisjointFun}
in the previous subsection, which says two function types are disjoint as long as
their return types are disjoint (regardless of their parameter types).
\rulelabel{DisjointAtomic} says two types are considered disjoint if they are
judged to be disjoint by the $A \not \sim B$ rules, which is going to be
explained below.
...
\begin{figure}
  \begin{mathpar}
    \framebox{$ \isdisjointi \Gamma A B$} \\
