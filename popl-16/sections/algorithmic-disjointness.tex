...
$A$ and $B$ are disjoint in a context $\Gamma$. \rulelabel{DisjointVar} is the
base rule and \rulelabel{DisjointVar} is its twin rule.
\rulelabel{DisjointInter1} and \rulelabel{DisjointInter2} inductively distribute
the relation itself over the intersection constructor ($\inter$). We have
derived \rulelabel{DisjointFun} in the previous subsection, which says two
function types are disjoint as long as their return types are disjoint
(regardless of their parameter types). \rulelabel{DisjointAtomic} says two types
