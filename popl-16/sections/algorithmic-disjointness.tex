...
rule is needed because disjointness is a symetric relation.
...
The rule for functions (\reflabel{\labeldisfun}) is interesting. It says that two function
...
function types do not any influence in determining whether those two function
types are disjoint or not: only the return types matter.

The rule for disjoint quantification (\reflabel{\labeldisforall}) is
also interesting. Consider the following two types:
...
form to ensure symetry. The rule \reflabel{\labelaxsym} takes care of that.

\subsection{Metatheory}

The algorithmic rules for disjointness are sound and complete.
\bruno{A bit of text and proof sketches are missing.}
