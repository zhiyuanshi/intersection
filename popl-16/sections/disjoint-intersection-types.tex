...
as well as type soundness. The result is calculus named \namedis. The keys
...
\begin{definition}[Disjoint types]
  ...
  C$: \footnote{ The definition of disjointness can also be adapted to type
  systems with a top type (such as \lstinline@Object@ in many OO languages): Two
  types $A$ and $B$ are \emph{disjoint} if: the fact that $C$ is a commmon
  supertype of theirs implies $C$ is the top type. }
...
satisfies the disjointness constraint\bruno{hummm, is the rule really
  enforcing that? do we miss a disjointness coinstraint}.
...
\begin{proof}
  By induction.
\end{proof}\bruno{by induction on what?}
...
\begin{lemma}[Unique subtype contributor]
  ...
  \end{enumerate}
\end{lemma}\bruno{missing proof sketch}
...
\end{restatable}\bruno{proof sketch}
...
\end{theorem}\bruno{missing proof sketch}
