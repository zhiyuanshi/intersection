...
\begin{theorem}[Unique elaboration] \label{theorem:unique-elaboration}
...
\begin{lemma}{Instantiation} \label{lemma:instantiation}
  If $\jwf {\Gamma, \alpha \disjoint B} C$, $\jwf \Gamma A$, $\jdis
  \Gamma A B$ then $\jwf \Gamma {\subst A \alpha C}$.
\end{lemma}

\begin{lemma}{Well-formed typing} \label{lemma:wf-typing}
...
\begin{lemma}[Unique coercion] \label{lemma:unique-coercion}
