...
Inductive and_coercion (e : SExp var) : PTyp -> sum (SExp var) (SExp var) -> Prop :=
  ...
  | ACBot : and_coercion e Bot (inr e)
...
Inductive sub : PTyp -> PTyp -> (SExp var) -> Prop :=
  ...
  | SBot : forall t, PType t -> sub Bot t (STLam _ (STUnit _))
...
Definition sand2 : forall t t1 t2, Sub t1 t -> PType t2 -> Sub (And t1 t2) t.
  ...
  - unfold Sub. unfold Sub in H. destruct H. inversion H.
    ...
    inversion H2.
    inversion H1; subst.
    assert (Ha1 : Sub (And Bot t3) t1) by (apply IHt1; unfold Sub; eauto).
    assert (Ha2 : Sub (And Bot t3) t2) by (apply IHt2; unfold Sub; eauto).
    destruct Ha1, Ha2; eauto.
  (* Case BVar *)
  - inversion H. inversion H1; inversion H3. inversion H2.
  (* Case FVar *)
  - apply sand2_atomic; auto.
  (* Case ForAll *)
...
Definition sand3 : forall t t1 t2, Sub t2 t -> PType t1 -> Sub (And t1 t2) t.
  ...
  - unfold Sub. unfold Sub in H. destruct H. inversion H.
    ...
    inversion H2.
    inversion H1; subst.
    assert (Ha1 : Sub (And t0 Bot) t1) by (apply IHt1; unfold Sub; eauto).
    assert (Ha2 : Sub (And t0 Bot) t2) by (apply IHt2; unfold Sub; eauto).
    destruct Ha1, Ha2; eauto.    
  (* Case BVar *)
  - inversion H; inversion H1; inversion H3. inversion H2.
  (* Case FVar *)
...
Proof.
  ...
  - dependent induction HusubAB; subst; auto.
    apply usub_lc in HusubCD; destruct HusubCD; auto.
  - dependent induction HusubAB; subst; auto.
  ...
  - dependent induction HusubCD; subst; auto.
    ...
    apply usub_lc in HusubCD; destruct HusubCD; auto.
    assert (Ha : ~ In x L0) by not_in_L x.
    apply H1 in Ha.
    apply usub_lc in Ha; destruct Ha; auto.
    apply usub_lc in HusubAB; apply USTop; destruct HusubAB; auto.
  - dependent induction HusubCD; subst; auto.
    apply usub_lc in HusubAB; apply USTop; destruct HusubAB; auto.
  - dependent induction HusubAB; subst; auto.
