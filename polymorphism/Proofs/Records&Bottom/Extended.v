...
Proof.
  ...
  - (* right; unfold not; intros HInv; inversion HInv. *)
    ...
    not_in_L x.
  - left; apply TLTop.
  - right; unfold not; intro HInv; inversion HInv.
...
Proof.
  ...
  - auto.
  - (* HOrtho should lead to False instantly... see below bot_ortho_top *)
    dependent induction HOrtho; subst.
    + assert (Ha : ~ TopLike t2 \/ ~ TopLike t3).
      apply Classical_Prop.not_and_or.
      unfold not; intro HH; destruct HH; apply HNotTL; auto.
      inversion H; inversion WFB; subst.
      destruct Ha.
      apply IHHOrtho1; auto. unfold Sub; eauto.
      apply IHHOrtho2; auto; unfold Sub; eauto.
    + inversion H1; subst.
      admit.
      admit.
      admit.
...
Lemma bot_ortho_top : forall Gamma A B, BottomLike A -> Ortho Gamma A B -> TopLike B.
Proof.
  unfold not.
  intros Gamma A B HBL HOrtho.
  induction HOrtho; intros; eauto.
  - inversion HBL; subst; auto.
  - inversion HBL; subst; auto.
  - inversion HBL; subst.
    apply_fresh TLForAll as x.
    apply H0 with (x := x).
  ...
  - inversion HBL.
  - inversion HBL.
  - admit. (* OrthoAx case. Maybe induction on HBL? *)
Admitted.
