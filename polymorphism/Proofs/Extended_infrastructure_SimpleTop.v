...
Require Import Extended_definitions_SimpleTop.
...
Lemma usub_toplike : forall D, TopLike D -> forall A, usub A D.
  intros D T. induction T; intros. apply USTop. apply USAnd1; auto.
Defined.
...
Proof.
  ...
  - dependent induction HusubCD; subst; auto.
    ...
    apply H3.
    not_in_L x.
  - apply usub_top in HusubCD. apply usub_toplike. auto. 
Qed.
...
Lemma ortho_toplike : forall ty, TopLike ty -> forall Gamma t0, Ortho Gamma t0 ty.
  intros ty T. induction T; intros. apply OTop2.
  apply OAnd2; auto.
Defined.
...
Proof.
  ...
  - (*pose (usub_trans _ _ _ HWFty H0 HSub). *)
    apply OVar with (A := A) ; auto. 
    apply usub_trans with (B := ty).  auto. auto. auto.
  ...
  - apply usub_top in HSub. apply ortho_toplike. auto.
...
Proof.  
  ...
  - apply OVar with (A := subst_typ_source z u A).
    ...
    not_in_L z.
    apply wf_gives_types_source in HWFu. auto. auto.
    apply wf_gives_types_source in HWFu. 
    apply subst_source_lc. auto. auto.
...
Proof.
  ...
  - assert (Ha : sumbool (x = z) (not (x = z))) by apply VarTyp.eq_dec.
    ...
    + simpl; apply EqFacts.eqb_neq in Ha; rewrite Ha.
      ...
      apply subst_source_lc. apply wf_gives_types_source in HWFu. auto. auto.
      apply subst_source_lc. apply wf_gives_types_source in HWFu. auto. auto.
  - assert (Ha : sumbool (x = z) (not (x = z))) by apply VarTyp.eq_dec.
    destruct Ha as [Ha | Ha].
    + assert (HWFEnv' : WFEnv Gamma) by now apply wf_gives_wfenv in HWFu.
