...
Require Import Extended_definitions_bot.
...
Proof.
  ...
  - dependent induction HusubAB; subst; auto.
    apply usub_lc in HusubCD.
    apply USBot; destruct HusubCD; auto.
  - dependent induction HusubAB; subst; auto.
  ...
  - dependent induction HusubCD; subst; auto.
  - dependent induction HusubAB; subst; auto.
    apply usub_lc in HusubCD; apply USBot; destruct HusubCD; auto.
  - dependent induction HusubAB; subst; auto.
    apply usub_lc in HusubCD; apply USBot; destruct HusubCD; auto.
  - dependent induction HusubCD; subst; auto.
  - dependent induction HusubCD; subst; auto.
    ...
    apply_fresh PType_ForAll as x.
    now inversion H3.
    assert (Ha : not (In x L0)). not_in_L x.
    apply H1 in Ha.
    apply usub_lc in Ha.
    now destruct Ha.
  - dependent induction HusubAB; subst; auto.
...
Lemma bottomlike_subst :
  forall t u z, PType u -> BottomLike t -> BottomLike (subst_typ_source z u t).
Proof.
...
Lemma fresh_bot :
  forall t x, ~(In x (fv_ptyp t)) -> BottomLike (open_typ_source t (PFVarT x)) ->
         forall y, ~(In y (fv_ptyp t)) -> BottomLike (open_typ_source t (PFVarT y)).
Proof.
  intros.
  destruct (VarTyp.eq_dec x y).
  - subst; auto.
  - rewrite subst_typ_source_intro with (x := x); auto.
    apply bottomlike_subst; auto.
Qed.
...
Lemma Ortho_usub_trans :
  forall Gamma u ty d,
    ...
    Ortho Gamma u ty.
Proof.
  intros Gamma u ty d HNotBot HWFty HOrtho HSub.
  generalize dependent ty.
  induction HOrtho; intros; eauto.
  - assert (Ha1 : not (BottomLike t2)) by
        (unfold not; intros; apply HNotBot; now apply BLAnd1).
    assert (Ha2 : not (BottomLike t3)) by
        (unfold not; intros; apply HNotBot; now apply BLAnd2).
    induction ty; try now (inversion HSub; subst; auto);
    inversion HWFty; inversion HSub; subst; auto.
  - assert (Ha : not (BottomLike t4)) by
...
Lemma bottomlike_eq_dec_forall_spec :
  forall L t,
    (forall x : elt,
       ~ In x L ->
       BottomLike (open_typ_source t (PFVarT x)) \/
       (BottomLike (open_typ_source t (PFVarT x)) -> False)) ->
  ((forall x : elt,
      ~ In x (union L (fv_ptyp t)) ->
       BottomLike (open_typ_source t (PFVarT x))) \/
   (forall x : elt,
      ~ In x (union L (fv_ptyp t)) ->
      (BottomLike (open_typ_source t (PFVarT x)) -> False))).
Proof.
  ...
  intros.
  apply H0.
  apply fresh_bot with (x := x0); auto.
  not_in_L x0.
  not_in_L x.
Qed.

    
Lemma bottomlike_eq_dec :
  forall t, PType t -> (BottomLike t) \/ (~ BottomLike t).
Proof.
  intros t HPType.
  unfold not.
  induction HPType.
  - right; intro HInv; inversion HInv.
  - right; intro HInv; inversion HInv.
  - destruct IHHPType2; auto.
    right; intro HInv; inversion HInv; subst; contradiction.
  - destruct IHHPType1; destruct IHHPType2; auto.
    right; intro HInv; inversion HInv; subst; contradiction.
  - apply bottomlike_eq_dec_forall_spec in H0.
    destruct H0.
    left; apply_fresh BLForAll as y.
    apply H0.
    not_in_L y.
    right; intros.
    inversion H1; subst.
    pick_fresh x.
    assert (Ha : not (In x L0)) by not_in_L x.
    apply H3 in Ha.
...
Lemma bottomlike_subst_eq_dec :
  forall z u ty,
    PType u ->
    PType ty ->
    BottomLike (subst_typ_source z u ty) \/ ~ BottomLike (subst_typ_source z u ty).
Proof.
  intros z u ty PTypu PTypty.
  apply bottomlike_eq_dec.
  apply subst_source_lc; auto.
Qed.

Lemma bottomlike_subst_or :
  forall z u t,
    PType t ->
    PType u ->
    BottomLike (subst_typ_source z u t) ->
    BottomLike u \/ BottomLike t.
Proof.
  intros z u t HPTypt HPTypu HBotSubst.
  induction HPTypt; simpl in *; auto.
  - destruct (eqb x z); auto.
  - inversion HBotSubst; subst.
    apply IHHPTypt2 in H0; inversion H0; auto.
  - inversion HBotSubst; subst.
    apply IHHPTypt1 in H0; inversion H0; auto.
    apply IHHPTypt2 in H0; inversion H0; auto.
  - inversion HBotSubst; subst.
    pick_fresh x.
    assert (Ha : not (In x L0)) by not_in_L x.
    apply H2 in Ha.
    rewrite subst_typ_source_open_source_var in Ha; auto.
    assert (Ha1 : not (In x L)) by not_in_L x.
    apply (H0 _ Ha1) in Ha.
    destruct Ha; auto.
    right; apply_fresh BLForAll as y.
    apply fresh_bot with (x := x); auto.
    not_in_L x.
    not_in_L y.
    not_in_L x.
...
Proof.  
  intros Gamma z u t1 t2 HNotIn HNotBotu HWFu HOrtho.
  ...
  - apply OVar with (A := subst_typ_source z u A); auto.
    ...
    unfold not; intro HBL.
    apply bottomlike_subst_or in HBL; auto.
    destruct HBL; contradiction.
    now apply wf_gives_types_source in HWFu. 
    apply subst_source_lc; auto.
    now apply wf_gives_types_source in HWFu.
...
Proof.
  intros z u Gamma d t1 t2 HNotIn HWFEnv HMapsTo HOrthoud HOrthot1t2 HWFu HWFt1 HWFt2.
  ...
  - assert (Ha : sumbool (x = z) (not (x = z))) by apply VarTyp.eq_dec.
    ...
    + assert (HWFEnv' : WFEnv Gamma) by now apply wf_gives_wfenv in HWFu.
    ...
    + simpl; apply EqFacts.eqb_neq in Ha; rewrite Ha. 
      assert (Ha1 : (BottomLike (subst_typ_source z u ty)) \/ (~ BottomLike (subst_typ_source z u ty))).
      apply bottomlike_subst_eq_dec; auto; now apply wf_gives_types_source in HWFu. 
      destruct Ha1. apply OBot2. auto.


      assert ((BottomLike (subst_typ_source z u A)) \/ (~ BottomLike (subst_typ_source z u A))).
      apply bottomlike_subst_eq_dec; auto.
      now apply wf_gives_types_source in HWFu.
      destruct H5.
      admit.
      apply OVar with (A := subst_typ_source z u A); auto.
  ...
  - assert (Ha : sumbool (x = z) (not (x = z))) by apply VarTyp.eq_dec.
    ...
    + simpl; apply EqFacts.eqb_neq in Ha; rewrite Ha.
      assert (Ha1 : (BottomLike (subst_typ_source z u ty)) \/ (~ BottomLike (subst_typ_source z u ty))).
      apply bottomlike_subst_eq_dec; auto; now apply wf_gives_types_source in HWFu. 
      destruct Ha1. apply OBot1. auto.  
      apply OVarSym with (A := subst_typ_source z u A); auto.
      apply in_persists_subst_env; auto.
      apply usub_subst; auto.
      now apply wf_gives_types_source in HWFu.
      unfold not; intros.
      admit. (* symmetry with previous case *)
      apply subst_source_lc; now apply wf_gives_types_source in HWFu.
      apply subst_source_lc; now apply wf_gives_types_source in HWFu.
...
Lemma subst_source_wf_typ_bot :
  forall t Gamma, WFTyp Gamma t -> forall z u d,
                       ...
                       MapsTo Gamma z d ->
                       WFTyp Gamma u ->
                       WFTyp Gamma (subst_typ_source z u t).
Proof.
  intros t Gamma Ht.
  induction Ht; intros; simpl; auto.
  - apply WFFun. eauto. eauto.
  - apply WFAnd; eauto.
    pose (IHHt1 _ _ _ H0 H1 H2).
    pose (IHHt2 _ _ _ H0 H1 H2).
    admit.
  - destruct (eqb x z); auto.
    eapply WFVar.
    apply H.
...
Lemma subst_source_wf_typ_bot' :
  forall t z u Gamma, not (In z (fv_ptyp u)) ->
             ...
             WFTyp (subst_env Gamma z u) (subst_typ_source z u t).
Proof.
  intros t z u Gamma HNotIn HMapsTo HWFEnv HWFu HWFt.
  induction HWFt; simpl; auto.
  - apply WFAnd; auto.
    admit.
  - assert (Ha : sumbool (x = z) (not (x = z))) by apply VarTyp.eq_dec.
    ...
    + apply EqFacts.eqb_neq in Ha.
      ...
      apply WFVar with (ty := subst_typ_source z u ty); auto.
      apply in_persists_subst_env; auto.
  - assert (Ha : WFTyp (subst_env Gamma z u) (subst_typ_source z u d)) by
        (apply IHHWFt; auto); clear IHHWFt.
    apply_fresh WFForAll as x.
    simpl in H0.
    rewrite subst_typ_source_open_source_var.
    ...
    rewrite union_spec in HH; destruct HH as [HH | HH]; not_in_L x.
    rewrite dom_subst_id; not_in_L x.
    rewrite <- app_nil_l with (l := (extend x (TyDis d) Gamma)).
...
Lemma open_body_wf_type_bot :
  forall t u Gamma, body_wf_typ t Bot Gamma -> WFTyp Gamma u ->
             WFTyp Gamma (open_typ_source t u).
Proof.
  intros. destruct H. pick_fresh y.
  assert (Ha : not (In y x)) by not_in_L y.
  ...
  not_in_L y.
  rewrite Ha1.
  apply subst_source_wf_typ_bot; eauto.
  not_in_L y.
  unfold MapsTo; simpl; now rewrite EqFacts.eqb_refl.
  rewrite <- Ha1.
  now apply wf_gives_wfenv in Ha.
  ...
  not_in_L y.
  apply WFBot.
  now apply wf_gives_wfenv in H0.
Qed.
 *)
