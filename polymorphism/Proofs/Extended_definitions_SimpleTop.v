...
(*
  | TLFun : forall t1 t2, TopLike t2 -> BottomLike (Fun t1 t2) (* what if t1 is bottomlike? should be bottom too ? *)
...
Inductive sub : PTyp -> PTyp -> (SExp var) -> Prop :=
  ...
  | STop : forall t, (*PType t ->*) sub t Top (STLam _ (STTApp _ (STBVar _ 0) (| t |))). (*FIXME!*)
...
Definition stop : forall t, Sub t Top.
  intros. unfold Sub. exists (STLam _ (STTApp _ (STBVar _ 0) (| t0 |))).
  apply STop; auto.
Defined.
...
Inductive WFTyp : context TyEnvSource -> PTyp -> Prop := 
  ...
  | WFBot : forall Gamma, WFEnv Gamma -> WFTyp Gamma Top.
...
(* Inference rules *)
  | ATyTop : forall Gamma, has_type_source_alg Gamma PTop Inf Top (STUnit _) 
