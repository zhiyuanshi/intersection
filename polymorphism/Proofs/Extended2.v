...
Fixpoint open_rec_typ_source (k : nat) (u : PTyp) (t : PTyp) {struct t} : PTyp :=
  ...
  | ForAll d t => ForAll (open_rec_typ_source (S k) u d)
...
(* TODO: these are too general for this version *)
...
Proof.
  ...
  - pick_fresh x.
    ...
    rewrite <- Ha1.
    assert (Ha2 : not (In x L)) by not_in_L x.
    apply H2 with (k := S k) in Ha2.
    apply open_rec_typ_source_core in Ha2.
    rewrite <- Ha2.    
    reflexivity.
    auto.
    auto.
...
Lemma ortho_weaken :
  forall G E F t1 t2, Ortho (E ++ G) t1 t2 -> WFEnv (E ++ F ++ G) -> Ortho (E ++ F ++ G) t1 t2.
Proof.
  intros.
  remember (E ++ G) as H'.
  generalize dependent HeqH'.
