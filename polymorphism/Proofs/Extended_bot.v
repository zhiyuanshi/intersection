...
Import Definitions.
*)
Require Import Extended_infrastructure_bot.
...
Lemma wfenv_no_var_sub :
  forall Gamma x A, WFEnv Gamma -> Sub A (PFVarT x) -> not (BottomLike A) ->
           not (List.In (x, TyDis A) Gamma).
Proof.
  intros.
  induction H.
  - auto.
...
Lemma ortho_no_sub :
  forall Gamma A B, WFTyp Gamma A -> WFTyp Gamma B -> not (BottomLike A) ->
           Ortho Gamma A B -> not (Sub A B).
Proof.
  intros Gamma A B WFA WFB HNotBot HOrtho HSub.
  ...
  - inversion HOrtho; subst.
    assert (Ha : not (BottomLike A)). admit.
...
Lemma uniquesub :
  forall Gamma A B C, WFTyp Gamma A -> WFTyp Gamma B ->
             not (BottomLike A) -> not (BottomLike B) ->
             Ortho Gamma A B -> Sub (And A B) C -> not (Sub A C /\ Sub B C).
Proof.
  intros Gamma A B C WFA WFB HNotBotA HNotBotB HOrtho HSubAnd.
  ...
  - induction C; try (now (inversion HSubA as [x HInv]; inversion HInv)).
    ...
    + inversion HSubA; inversion H1; subst.
      ...
      unfold not; intros; apply HNotBotA;
      apply_fresh BLForAll as y; apply fresh_bot with (x := x); auto.
      not_in_L x.
      not_in_L y.
      unfold not; intros; apply HNotBotB;
      apply_fresh BLForAll as y; apply fresh_bot with (x := x); auto.
      not_in_L x.
      not_in_L y.
      apply sand2.
  ...
  - destruct H as [ PTypHd1 [ PTypHd2 PTypHd3 ]].
    ...
    + inversion HSubA as [x HInv1]; inversion HInv1; subst; try (now inversion H0).
      ...
      apply IHC1; [ apply sand2; unfold Sub; eauto
                  | unfold Sub; eauto
                  | unfold Sub; eauto ].
      now apply HNotBotB.
      now apply HNotBotA.
    + inversion HSubA as [x HsA]; inversion HsA; subst; try now inversion H0.
      now apply HNotBotA.
    + inversion HSubA as [x HsA]; inversion HsA; subst; try orthoax_inv PTypHd1.
    + inversion HSubA as [x HsA]; inversion HsA; subst; try orthoax_inv PTypHd1.
      inversion HSubB as [x HsB]; inversion HsB; subst; try orthoax_inv PTypHd2.
