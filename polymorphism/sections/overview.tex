...
\bruno{We may be able to skip
  this paragraph and just have a quick sentence about the relationship
with pairs by pointing to Dunfield's paper. The important message is 
that with intersections the elimination is implicit, and the order of
the values is not important (with disjoint intersections).}
...
$\tyint \inter \tyint$.
For example, what should be the result of this program:
\begin{lstlisting}
(fun (x: Int) (*$ \to $*) x) (1,,2)
\end{lstlisting}
Should the result be \lstinline$1$ or \lstinline$2$?
The \oldname \joao{add reference if not mentioned before} system solves this
...
This is precisely the approach taken in \oldname: \joao{add reference here} 
...
For instance, the previous example: \bruno{those are old examples (pre-ICFP) by
  the way; and they do not work in the calculus we have (no
  annotations on lambda's)}
\begin{lstlisting}
(fun (x: Int) (*$ \to $*) x) (1,,2)
\end{lstlisting}
...
\subsection{Parametric Polymorphism}\bruno{avoid writting
  ``we'', ``I'', ``us'', \ldots when possible. Start with a key sentence, that
  summarizes your point. For example: 
``Unfortunatelly, adding parametric polymorphism is non-trivial. A naive
attempt to add polymorphism ...''
}
\bruno{Be consistent with the title(s) capitalization of words. Some
  titles have lower case words, some others upper case. Use upper case
everywhere (except for words like ``the'' ``of'', etc.}
Unfortunately, adding parametric polymorphism is non-trivial.
A naive attempt to add polymorphism would consist in introducing a forall type, type variables, and a big lambda 
at term level.
...
\subsection{A Conservative Attempt}
A first attempt at restoring coherence can be to forbid type variables inside
...
\bruno{There was an important discussion about biased choice in the
  draft, but this seems to have been removed. I think that's a very
  important point as to why we coherence is important. Furthermore, 
it is related to type substitution and why stability of subtitution
...
\joao{this is somewhat similar to what is already written on Section 5, maybe merge them here?}
...
\paragraph{Dropping the specification of disjointness}
\bruno{I think the discussion is too technical here. Leave the more
  technical details for latter. You want to use concrete programs as
  examples here, rather than having a technical discussion. You want
  to say that a type variable with no constraints is a syntactic sugar
  for a top constraint. }
Another issue that arises from disjoint quantification, is that the specification is actually no longer valid.
Variables are now disjoint to some type, up until instantiation, where they might become a 
concrete type.
This hints that we should refer to unification in the specification. \joao{add example here?} 
However, the specification was used partially to provide a more intuitive look at disjointness, due to its
simplicity.
We argue that, using unification in the specification will work against this principle, and hence
\name will only work with the disjointness implementation. \bruno{No
  discussions about specification here.}

\paragraph{Substitution}
\joao{how should we cover this here? it is already explained in
  Section 5}
\bruno{Connect it to the example that was in the earlier draft, which 
illustrated that can substitution change the semantics, if we are not careful.}
