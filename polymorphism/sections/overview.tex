...
merge operator and pairs is in the elimination construct. 
...
Also, with \emph{disjoint} intersection types, the order of the types in the
intersecions becomes irrelevant.
\joao{missing something here}
...
$\tyint \inter \tyint$.
For example, what should be the result of this program: \joao{missing anotation here?}
\begin{lstlisting}
(fun x (*$ \to $*) x) (1,,2)
\end{lstlisting}
...
multiple meanings.
The \oldname \joao{add reference if not mentioned before} system, a calculus with
...
With this concept of disjointness in mind, it is easy to verify that there is no longer
incoherence in the previous example, since 
\bruno{those are old examples (pre-ICFP) by
...
\begin{lstlisting}
let fst A B (x: A & B) : A = (fun z (*$ \to $*) z) x in (*$ \ldots $*)
\end{lstlisting}
...
Even though disjoint quantification solves the problem of coherence, there is still one problem
we need to tackle. 
...
Thus, the more specific a type variable is, the less options we have to instantiate it with.
