...
\oldname is a simply-typed calculus with intersection types and a merge operator, 
in the same flavour of Dunfield's, but requires two types composing an intersection 
to be \emph{disjoint}.  
Disjointness is a binary relation between two types which tells whether they contain
overlapping types.
...
\joao{maybe show here how top was supported? since top plays a role in this calculus too}

\subsection{Adding parametric polymorphism}

Let us suppose that we add parametric polymorphism to \oldname in a very straightforward 
and naive way, by introducing a forall type, type variables, and a big lambda at term level.
We also say that type variables are disjoint to any other type, as a starting point.
...
Naturally a first attempt at restoring coherence would be to not allow type variables inside
...
For example, function $fst$ defined above, would no longer be accepted by our system.
...
in respect to other standard system, i.e. System $F$.
...
A possible approach would be to consider assigning constraints to each type variable, an delay the 
...
\joao{explain why we dropped it: unification}

\paragraph{Changing the notion of top-like types}

\joao{in constrast with the old calculus, top is now disjoint to anything}

\paragraph{The resulting calculus}
