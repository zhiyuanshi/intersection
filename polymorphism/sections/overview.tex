...
on integers and characters, respectively -- both accept \lstinline{x} as an argument. 
...
\begin{lstlisting}
let x : Int & Bool = 1,,True in (*$ \ldots $*)
\end{lstlisting}
...
multiple meanings.  The \oldname~\cite{oliveira16disjoint} calculus --
...
the types in the intersection overlap. However an intersection 
such as \lstinline{Char&Int} is ok because the set of characters 
and integers do not overlap with each other. 
...
intersection types is non-trivial. Consider the following programs
(we will use uppercase Latin letters to denote type variables):
...
\noindent The first program, \lstinline{merge3}, takes an argument
...
overlaps with \lstinline{Int}, then incoherence may happen.

\begin{lstlisting}
merge3 Int 2
\begin{lstlisting}

\noindent can result in both 2 or 3. 

Unfortunately, combining parametric polymorphism with disjoint
intersection types is non-trivial. For example, consider the following program:

\begin{lstlisting}

\end{lstlisting}

A naive attempt to add polymorphism consist in forbidding type
variables to appear in an intersection. 
...
\subsection{Disjoint Quantification}\label{subsec:disjoint-quantification}
