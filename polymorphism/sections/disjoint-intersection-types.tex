...
We will first explain the semantics by means of the elaboration to System $F$.
Then, we will discuss the necessary extensions to retain coherence, namely in the coercions of 
...
It is important pointing out that, despite the similarity of these rules with the simply-typed 
system, our notion of disjointness has changed.
Consequently, the set of well-formed top-like types has changed and
we also adjusted the meta-function $\andcoerce{A}$, as shown in the bottom of 
Figure~\ref{fig:andcoercion}.
Note how not only the $\forall$ case is defined, but also the intersection case.
...
coherent.
More formally, with disjoint intersections the following theorem holds:
