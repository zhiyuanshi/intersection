...
According to~\cite{oliveira16disjoint}, some top-like types are problematic,
for instance: $\top \inter \top$.
Not only this type is a super-type of every other type (including $\top$), but
also it might impose a coherence problem, as both components of the
intersection overlap.
However, taking a closer look at it clarifies that such issues are not problematic. 
To explain why, let us first consider the ill-type $\tyint \inter \tyint$.
...
when both terms composing the merge are equivalent (i.e. $\beta$-equivalent), as in 
$(2 \mergeop 2) : \tyint$.
...
do not overlap.
Generally speaking, top-like types should never be a source of incoherence, implying that 
%Thus, in contrast with \cite{oliveira16disjoint}, they can be safely intersected with any other type.
$\top$ is disjoint to every other type (as mentioned in Section~\ref{sec:alg-dis}).
...
The only new rule is \reflabel{\labeltoplforall}, which states that a (disjoint) universal quantifier 
