...
\joao{from what I could gather we use it to ensure well-formed typing, which is used here (as mentioned below)}
\begin{proof}
...
\noindent we need to show that $E_{sub}$ is unique (by
  Lemma~\ref{lemma:unique-coercion}), and thus to show that $A$ is well-typed 
  (by Lemma~\ref{lemma:wellformed-typing}). 
