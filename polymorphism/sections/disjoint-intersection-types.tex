\section{Semantics, Coherence and Type-Safety}
\label{sec:disjoint}
This section discusses the elaboration semantics of \name and proves
type-safety and coherence.
%The semantics is by an elaboration to System $F$.
...
\end{figure}\bruno{This figure is too wastefull in terms of
  syntax. Use a table or tabular to put the two translations
  side-by-side. The Figure should take half of the space that it
  currently takes.}
...
An important set of types are the so-called top-like types.
These are types that somehow resemble $\top$ and the importance in classifying
them is crucial when it comes down to proving coherence.
This is due to top-like types only having one inhabitant each and therefore 
their (unique) coercions can be directly generated from their structure. 
For example, the term $\lam x \unit$ represents the only inhabitant of the type 
$\tyint \to \top$.

However, according to~\cite{oliveira16disjoint}, some top-like types might be
problematic.
For instance, a top-like type can be an intersection composed of $\top$ types, as in
$\top \inter \top$.
This type is a super-type of every other type, including $\top$ itself.
Also, there might be a coherence problem with this type, since both components of the
intersection overlap.
However, taking a closer look at this type ensures that there is no problem with
such types.
To see this, let us first consider the ill-type $\tyint \inter \tyint$.
This type is a potential source for incoherence, as both components of the merge
overlap.
%The $\tyint \inter \tyint \subtype \tyint$ judgement has to produce a coercion which
%is biased, and therefore might lead to different results.
For example, the term $(3 \mergeop 4) : \tyint$ leads to incoherency, since 
both terms $3$ and $4$ have type $\tyint$. 
There is one case though, where coherence would not be compromised:
whenever the inhabitants of both types are the same, as in $2 \mergeop 2$.
In this case, both values can be chosen interchangibily, without affecting the coherence
of the program.
It is important realizing that this is always the case with top-like intersections.
%In fact, this explains why $\top$ is actually disjoint with every other type.
Moreover, top-like types can also be safely intersected with other types, as their inhabitants
do not overlap.
More generally, top-like types should never be a source of incoherence. 
Thus, in contrast with \cite{oliveira16disjoint}, they can be safely intersected with any other type.
This also explains why $\top$ is disjoint to every other type, as mentioned in Section~\ref{sec:alg-dis}.
...
the (disjoint) universal quantifier.
...
\begin{proof}
By induction on the first derivation and case analysis \joao{or say inversion instead?} on the shape of the second. 
\end{proof}
...
$\yields {}$.  The translation ensures that $E$ has type $\im A$.  We
discuss the most relevant rules/coercions in greater detail next.  The
two new rules for type abstraction (\reflabel{\labeltblam}) and type
application (\reflabel{\labelttapp}) generate the expected
corresponding coercions in System F.  In \name, subtyping means, for
...
coerce the System F term into the right type.  The coercions generated
for \reflabel{\labeltrec} and \reflabel{\labeltprojr} simply erase the
labels (since there are no labels in the target language) and
translate the corresponding underlying term. It is also noteworthy
