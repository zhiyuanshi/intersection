...
\label{sec:disjoint}
\bruno{You are repeatedily referring to our previous paper and Dunfield's paper. 
You don't need to constantly remind the reader of this. Remove some of this repetition.}
...
\joao{actually they can't, but should we include them?}
...
It is important pointing out that, despite the similarity of top-likes types,  
the notion of disjointness has changed, leading to a larger set of well-formed top-like types.
...
as $\top \inter \top$).
This changes play a fundamental role in ensuring the coherence of subtyping, as we will show next.
...
Rule \reflabel{\labelsubtop} the coercion is the constant function of the unit term.
...
In order to ensure a coherent type-system, we should prove that our subtyping relation is also 
coherent.
...
The variable cases \joao{ref the var rules instead} needed to show that, for any two well-formed and disjoint types 
$A$ and $B$, and $B$ is not toplike, then $A$ cannot be a subtype of $B$.
\end{proof}
...
\bruno{Don't just copy\&paste from the previous paper. 
You should mention the new rules, which Dunfield does not have. }
...
Rule \reflabel{\labeltblam} introduces a type variable $a$ into context, and naturally does not 
...
To account for this difference, in \reflabel{\labeltapp}, the
coercion $E$ from the subtyping relation is applied to the argument.
Rules \reflabel{\labeltrec} and \reflabel{\labeltprojr} are quite straightforward, since there
are not records in the target language.
It might also be noteworthy saying that, as usual, the rule \reflabel{\labeltmerge} translates merges 
into pairs.
...
\joao{review this proof}
\bruno{Yes, and you want to mention somewhere where the stability of substitution lemma 
plays a role. I presume it plays a role in the coherence theorems.}
\begin{proof}
