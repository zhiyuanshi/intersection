...
\bruno{This section needs some major rewriting. Remeber that the goal
  of writting the related work section is to identify how previous
  work is different from ours. The text written here is also slightly
  outdated compared to what we wrote on ICFP. In particular we discuss
the work by Castagna on merges at ICFP.}
...
Our work is an extension to the recent \oldname~\cite{oliveira16disjoint}.
\name solves the coherence problem present in Dunfield's calculus, by requiring that
...
disjoint as long as they do not share a common supertype.
\name does not use such definition as its adaptation to polymorphic types would require
using some form of unification.
Since the purpose of such specification is its simplicity and readability (compared to the
algorithmic counterpart), we argue that unification would go against this principle, and
opted to drop the specification.
In fact, \name's notion of disjointness is based on \oldname's definition of the more
general notion of disjointess concerning $\top$ types, called $\top$-disjointness. 
The definition says that, two types $A$ and $B$ are disjoint if two conditions are satisfied:
...
As a consequence, the set of well-formed \emph{top-like} types is a superset of \name's.
The reason for this is covered in greater detail in Section~\ref{} \joao{fix ref}.
...
The difference to our calculus, is that their intersections are mostly used at function
level, allowing overloading (i.e. branching) of types.
For example, they can express a function that works on any type, but executes different
code depending on the type of its input:
...
\noindent In our system we cannot express such intersections.
However, \name does accept other kind of intersections which are not possible to express
in their calculus, as in merges of type $(\tyint \to \tybool) \inter (\tyint \to \tyint)$.
Just as in~\cite{Castagna92calculus}, their system is focused on the combination of 
...
as it has been discussed in Sections~\ref{} and Section~\ref{}. \joao{fix refs} 
...
Most calculi are based on these three primitive operators (especially extension). \joao{add references}
...
However, in \name, the polymorphic extensible records implementation piggybacks on the disjointness
and intersection types features of the type-system.
Thus, the constraints on labels are encoded using disjoint quantification, and the coherent
...
Traits and mixins have become very popular in object-oriented languages. \joao{references}
They allow for some type of object composition which is not allowed in
...
usually language dependent. \joao{is this true?}
...
names/labels are always rejected in the former, whereas in the latter only
if their types are disjoint.
\joao{do you want to reference CZ, where they have a similar spec to our notion of disjointess?}
