...
supports both the merge operator (called $glue$ in $F_\wedge$), intersection types and polymorphism.
However $F_\wedge$ is incoherent. Various simply typed systems with 
...
types.\bruno{I think you need to comment on the relation between
  disjointness and negation ($\alpha \backslash \tyint$)}
...
Most calculi are based on these three primitive operators (especially extension)
...
disjointness in \name enables a model with a phylosophy similar to
