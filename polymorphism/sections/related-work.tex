...
\joao{maybe these 3 paragraphs can go, as they aren't refering polymorphic systems?}
...
general notion of disjointess concerning $\top$ types, called $\top$-disjointness, which 
...
$ F_{\subtype \rho}$, with extensible records to a simpler calculus without
...
This means that technical concerns, i.e type inference, efficiency, etc.; are to be dealt with the 
...
\bruno{Moving some text here, since it seems more adequate for related
work. 
...
As noted in~\cite{leijen2005extensible}, systems typically can be categorized into two distinct groups
