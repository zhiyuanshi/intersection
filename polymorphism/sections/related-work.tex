...
To our knowlegde no previous work presents a \emph{coherent} calculus
which includes parametric polymorphism, intersection types and a
\emph{merge} operator. As far as we are aware only Pierce's
$F_\wedge$~\cite{pierce1991programming2}\bruno{joao: you need to read
  Pierce's stuff to double-check that what I say is correct.}
supports both the merge operator, intersection types and polymorphism.
However $F_\wedge$ is incoherent.  In $F_\wedge$ the
merge operator is called $glue$. Various simply typed systems with 
...
However, \name accepts other kinds of intersections which are not possible to express
...
arbitrary types.\bruno{I think you need to comment on the relation
  between disjointness and negation ($\alpha \backslash \tyint$)}
...
object-oriented languages such as Scala, TypeScript, Flow, Ceylon, and
Grace\bruno{references}.  There is also a foundational calculus for Scala
that incorporates intersection types~\cite{rompf2016soundness}.
Generally speaking, the most significant difference between \name 
and those languages/calculus is that
in they have no explicit introduction construct
like our merge operator.  This leads to several ad-hoc solutions at
defining a merge operator in those languages, as it has been discussed in
Section~\ref{sec:intro} and Section~\ref{subsec:mixins}.
