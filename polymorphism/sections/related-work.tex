...
\oldname uses a specification for disjointness, that reads as:
...
A major difference between the two calculus, is that $\top$-disjointness does not allow
...
The reason for this is covered in greater detail in Section~\ref{} \joao{put ref to section}.
...
records that became standard in type-systems with record types: 
\emph{selection}, \emph{restriction}, and \emph{extension}. 
Examples of calculus that use such primitives are ...
...
duplicate labels. 
Leijen's system is more sophisticated, since it relies on shadowing of labels. 
...
Later ... defined a fourth operation on records called \emph{concatenation}.
The operation eventually became standard of many systems, even though its semantics can be usually
defined in terms of extension.
One problem with concatenation is that tipically systems are hard to use in practise, due to the
...
The merge operator in \name plays the same role as concatenation, as its components may contain
any (well-formed and disjoint) types.
However.. \joao{type signatures are simpler and piggybacking on more general notion(s) of DIT}
