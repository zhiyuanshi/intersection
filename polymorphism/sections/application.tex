...
The implementation of \lstinline{extend} relies on a biased choice.
Given two arguments \lstinline$first$ of type \lstinline$T$, and \lstinline$second$ of type 
\lstinline $U$, \lstinline{extend} returns an intersection of both types represented as \lstinline$T \& U$.
...
and thus there is no need to define \lstinline{extend}.
In fact, the introduction of disjoint intersection types can be seen as a type-safe 
and coherent solution for all of the mentioned problems involving intersection types. 
\joao{mention the diamond problem? and that two fields with same name can be composed as long as
...
well-typed in \name.
Moreover, the intersection \lstinline$Person & Dog$ would be ill-typed, since the types share the
same field \lstinline$name$. \bruno{please write down the full code in
\name to illustrate how to do the example.}
...
cases \cite{blume2006extensible}; amongst others.
...
%\end{align*}
\begin{lstlisting}[mathescape=true]
let $\text{average}_1$ (R\y, R\y) => (r : { R | x : Int, y : Int }) : Int = (r.x + r.y) / 2 in $\ldots$ 
\end{lstlisting}
...
%\end{align*}
\begin{lstlisting}[mathescape=true]
let $\text{average}_3$ (R * { x : Int } & { y : Int }) (r : { x : Int } & { y : Int } & R) : Int = (r.x + r.y) / 2 in $\ldots$ 
\end{lstlisting}
