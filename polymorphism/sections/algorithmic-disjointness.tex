...
\noindent The lemma states that if a type $A$ is disjoint to $B$ under $\Gamma$, then it is also disjoint
to any supertype of $B$. 
Note how these two variable rules would allow one to prove $x \disjoint x$, for any type 
variable $x$.
However, under the assumption that contexts are well-formed, such derivation is not possible 
as $x$ cannot occur free in $A$. 

The rule for disjoint quantification \reflabel{\labeldisforall} is the last novel rule. 
It adds a constraint composed of the intersection of both constraints into $\Gamma$ and checks for 
...
As the reader might expect, the answer is tightely related to subtyping. 
...
Following the same logic, we may conclude that having $\top$ (i.e. the 0-ary intersection)
