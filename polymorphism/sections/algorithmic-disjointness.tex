...
\bruno{The labels of the first two rules
...
\joao{how about quickly recalling the old specification here?}
\bruno{I think we should only mention the specification in related work.}

The rules for the disjointness judgement are shown in
Figure~\ref{fig:disjointness}, which consists of two judgements.
 
\paragraph{Main judgement.} The judgement $\jdisimpl \Gamma A B$\bruno{Use $*$ instead 
of $*_i$ since there's only one version of disjointness in this paper.} says
...
\bruno{Here you want to mention the interesting use of intersection types in the forall 
rule: disjointness constraints may contain non-disjoint intersection. You should explain
why this is is needed and why it does not introduce incoherence.}
...
\joao{mention here disjointness is stable by substitution (copy ortho\_subst)}
