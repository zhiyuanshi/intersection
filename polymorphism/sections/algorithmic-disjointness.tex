...
$\top$.\bruno{remove brackets from rule names} Type variables are
...
\joao{can we give this a name, as it is re-stated later?
  i.e. covariance of disjointness?}\bruno{Yes, covariance of
  disjointness is a good name.}
...
well-formedness and typing.  \bruno{we probably want a quick reference
  to some example in the overview illustrating this issue here.}
However, under certain conditions, a weaker version of the substituion
lemmas does hold. The conditions are guaranteed by the type-system by only
...
for disjointness of variables:\bruno{if you give the lemma a name, you
can simply use the name here.}

\[ \inferrule {\jwf \Gamma {A \disjoint B} \\ B \subtype C }
              {\jwf \Gamma {A \disjoint C}} \]

If one takes $A$ as some variable $x$, and $B$ has some disjointness constraint bound to it in 
...
shape of $B$?\bruno{here you are talking about some example with x and
B with constraint in $|gamma$. I think it is better to have a rule
that illustrates this directly (as a special case of the rule above).}
...
translation of $T_{F}$.
\joao{should we include here a simple example? i.e. the identity
  function?}
\bruno{One example can be presented in the overview, perhaps.}


\paragraph{The most restrictive bound}\bruno{I think the discussion
  about bottom can be done later, perhaps in future work.}
