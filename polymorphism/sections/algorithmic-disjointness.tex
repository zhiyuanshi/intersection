...
\paragraph{Stability of substitutions}
...
\paragraph{Well-formedness and substitution}
...
\paragraph{Upper-bound}\joao{maybe rename this? it is the most liberal bound}
...
However, introducing $\bot$ is not compatible with our disjointness definition, due to the rule
\reflabel{\labeldisvar}.
Notice that if $A$ is $\bot$, then it is not true that $A$ is disjoint to all super-types of
$\bot$ (i.e. all other possible types).
To understand why, let us recall the reader of the property of which \reflabel{\labeldisvar} is
based on:

\[ \inferrule {\jwf \Gamma {A \disjoint B} \\ B \subtype C }
              {\jwf \Gamma {A \disjoint C}} \]

If one instantiates $A$ with a variable $\alpha$ and, most importantly, $B$ with $\bot$, the
lemma still holds, since $\alpha \disjoint \bot$ is false!
This suggests that we should not be able to introduce $\bot$ in the environment, as we are
constructing evidence for a false hypothesis.
While this could be a straightforward restriction in our system, one might ask: what about
introducing $\bot \inter \bot$ in the environment? 
Or, more generally, introducing any \emph{bottom-like} type? 
Since defining the lower bound is not strictly necessary to our formalization; introduces 
substantial complexity in our system; and its practical application is not clear, we left this as 
