...
\noindent whose type signature involves an intersection between 
types \lstinline{B} and \lstinline{C}. In Java, such method cannot  
be directly expressed. 

The existence of first-class intersections has also lead to the
discovery of other interesting applications of intersection types. 
For example, in their documentation of intersection types\footnote{\url{https://www.typescriptlang.org/docs/handbook/advanced-types.html}}, 
TypeScript motivates them as follows:
...
support parametric polymorphism. This is a pressing limitation because
...
lack of coherence is an important disadvantage for adoption of his core
...
($A * B$)
%\footnote{The notation $A * B$ is inspired by the
%  \emph{separating conjunction} construct in separation
%  logic~\cite{}.}
...
Disjoint intersection types can be extended to
support parametric polymorphism. However, parametric polymorphism makes the
...
We also investigate how to do type-theoretic encodings of datatypes in
\name. In particular it is shown that extensions of datatype
encodings have subtyping relations with the datatype they
extend. Moreover, it is possible to reuse code from the operations on
the original datatype and consequently solve the Expression Problem.
Finally, it is shown how \emph{all the features} of \name
(intersection types, the merge operator, parametric polymorphism and
disjoint quantification) are needed to properly encode one important
combinator~\cite{oliveira2013feature} used to compose multiple
operations over datatypes.
