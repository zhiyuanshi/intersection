...
type \lstinline{B with C} weaken the type to \lstinline{B}. 
...
calculi~\cite{reynolds,castagna,dunfield2014elaborating,oliveira} as an introduction
form for intersection types. As we have seen the merge operator is
encodable in TypeScript using low-level features. Similar encodings 
have been proposed for Scala to enable applications where 
the merge operator plays a fundamental
role~\cite{}. Unfortunatelly, the merge operator is not directly supported
by TypeScript, Scala, Ceylon or Flow. A possible reason for this is
that most calculi with intersection types in the literature lack the
merge operator. While powerful, the merge operator is 
well-known to introduce \emph{(in)coherence} problems~\cite{}. 
If care is not taken, certain programs using the merge operator do 
not have a unique semantics, which significantly complicates reasoning 
about programs.   

Recent work proposed using \emph{disjoint intersection types}~\cite{oliveira16disjoint} to 
...
selecting a value of the appropriate type from an intersection, which 
guarantees coherence. However, such work was done in a simply typed
setting. 

Combining parametric polymorphism wiht disjoint intersection
...
programs, including the \lstinline{extend} function. 
...
coherence. In our calculus the \lstinline{extend} function trivially implemented
as follows:
