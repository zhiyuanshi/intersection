...
% \noindent {\bf commutativity: } $A \intersect B = B \intersect A$  \\
% {\bf associativity:~~~~~~}$A \intersect (B \intersect C) = (A \intersect B) \intersect C$
...
% \lstinline{e1} and \lstinline{e2} are two terms that support both
% evaluation and pretty printing and each has type \lstinline{eval : Int, print :
%   String}. \lstinline{add} takes two terms and computes their
...
% that the two terms support evaluation and hence the type of the parameter
