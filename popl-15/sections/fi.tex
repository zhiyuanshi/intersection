...
\rulelabel{SubAnd} says that if $\tau_1$ is both the subset of $\tau_2$ and the
subset of $\tau_3$, then $\tau_1$ is also the subset of the intersection of
$\tau_2$ and $\tau_3$. In order to achieve coherence, \rulelabel{SubAnd1} and
...
intersection constructor ($\intersect$). \rulelabel{DisjointFun} is quite
...
variables (\rulelabel{Var}), lambda abstractions (\rulelabel{Lam}), type
abstractions (\rulelabel{BLam}), and type applications (\rulelabel{TApp}). For
the ease of discussion, in \rulelabel{BLam}, we require the type variable
...
The rule \rulelabel{App} needs special attention as we add a subtyping
