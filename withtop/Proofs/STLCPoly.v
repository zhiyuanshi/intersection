...
Fixpoint open_rec_type (k : nat) (u : STyp) (t : STyp) {struct t} : STyp :=
  match t with
  | STUnitT       => STUnitT
  | STInt         => STInt
  | STFun t1 t2   => STFun (open_rec_type k u t1) (open_rec_type k u t2)
  | STTuple t1 t2 => STTuple (open_rec_type k u t1) (open_rec_type k u t2) 
  | STFVarT x     => STFVarT x
  | STBVarT i     => if Nat.eqb k i then u else (STBVarT i)
  | STForAll t    => STForAll (open_rec_type (S k) u t)
  end.

Fixpoint open_rec_type_term (k : nat) (u : STyp) (t : SExp var) {struct t} :
  SExp var :=
  match t with
  ...
  | STUnit _ => STUnit _
  | STLit _ x => STLit _ x
  | STLam _ ty t1 => STLam _ (open_rec_type k u ty) (open_rec_type_term k u t1)
  | STLam' _ t1 => STLam' _ (open_rec_type_term k u t1)    
  | STApp _ t1 t2 => STApp _ (open_rec_type_term k u t1) (open_rec_type_term k u t2)
  | STPair _ t1 t2 => STPair _ (open_rec_type_term k u t1) (open_rec_type_term k u t2)
  | STProj1 _ t => STProj1 _ (open_rec_type_term k u t)
  | STProj2 _ t => STProj2 _ (open_rec_type_term k u t)
  | STTLam _ t => STTLam _ (open_rec_type_term (S k) u t)
  | STTApp _ t ty => STTApp _ (open_rec_type_term k u t) (open_rec_type k u ty)
  end.
...
Definition open_typ (t : STyp) u := open_rec_type 0 u t.
Definition open_typ_term (t : SExp var) u := open_rec_type_term 0 u t.
...
Inductive STType : STyp -> Prop :=
  ...
  | STType_Fun : forall t1 t2, STType (STFun t1 t2)
  | STType_Tuple : forall t1 t2, STType (STTuple t1 t2)
...
Proof.
  ...
  - rewrite <- IHSTTerm.
    reflexivity.
  - pick_fresh x. Check open_rec_term_core.
    admit.
    (*
    rewrite <- open_rec_term_core with (j := (S k)) (v := STFVar _ x).
    reflexivity.
    auto.
    apply H0.*)
  - rewrite <- IHSTTerm.    
...
Proof.
  ...
  - apply_fresh STTyLam as x.
    ...
    inversion H7; exfalso; contradiction.
    auto.
  - apply_fresh STTyLam' as x.
    unfold open in *; simpl in *.
    subst.
...
Proof.
  ...
  - subst.
    ...
    not_in_L x.
    not_in_L z.
    apply fv_distr in H3; rewrite union_spec in H3; inversion H3.
    auto.
  ...
  - subst.
    ...
    admit.
(*
    apply fv_distr in H4. rewrite union_spec in H4; inversion H4.
    auto.
    assert (NeqXZ : not (In x (singleton z))) by (not_in_L x).
    simpl in H3.
    inversion H3.
