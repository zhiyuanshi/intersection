...
Fixpoint open_rec_source (k : nat) (u : PExp) (t : PExp) {struct t} : PExp  :=
  ...
  | PAnn m e t     => PAnn m (open_rec_source k u e) t
  end.
...
Inductive has_type_source : context PTyp -> PExp -> PTyp -> PExp -> Prop :=
  ...
  | TyLam : forall L Gamma t t1 A B, (forall x, not (In x L) -> 
                           ...
                           WFTyp A ->  
                           has_type_source Gamma (PLam t) (Fun A B) (PAnn CT (PLam t1) (Fun A B)) 
  ...
  | TySub : forall Gamma t t' A B,
              ...
              Sub A B ->
              WFTyp B ->
              has_type_source Gamma t B (PAnn CT t' B).
...
Fixpoint subst_source (z : var) (u : PExp) (t : PExp) {struct t} : PExp :=
  match t with
    ...
    | PAnn m t1 t2  => PAnn m (subst_source z u t1) t2 
  end.
...
Proof.
  ...
  eexists. apply H0.
  eexists. apply H0.
  exists (PAnn CT (PLam t2) (Fun A0 B0)).
  rewrite <- open_rec_source_term.
  reflexivity.
...
Definition tlam : forall L Gamma t A B, (forall x, not (In x L) -> 
  ...
  exists (PAnn CT (PLam x0) (Fun A B)).  
  apply_fresh TyLam as z. 
...
unfold has_type. intros. destruct H. exists (PAnn CT x B). apply (TySub _ _ _ A); auto.
Defined.  
...
Definition tyann : forall Gamma t1 A, has_ty Gamma t1 Chk A -> has_ty Gamma (PAnn CT t1 A) Inf A.
