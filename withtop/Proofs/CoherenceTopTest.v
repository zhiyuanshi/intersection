...
Hint Constructors TopSig TopLike sub Atomic.

Lemma topsig_gives_toplike : forall t, TopSig t -> TopLike t.
Proof.
...
Lemma topLikeSub : forall A B, Sub A B -> TopLike A -> TopLike B.
Proof.
  intros A B [x HsubAB] HTLA.
  induction HsubAB; auto; inversion HTLA; subst; auto.
Qed.
...
Proof.
  ...
  - destruct H0.
    ...
    inversion H3; subst.
    apply TLFun.
    apply topLikeSub with (A := o2).
    unfold Sub; eauto.
    now apply topsig_gives_toplike.
    destruct H1; inversion H1; subst; try now inversion H5.
    apply TLAnd.
    apply IHsub1; auto.
    unfold Sub; eauto.
    apply IHsub2; auto.
  ...
  - destruct H1.
    ...
    unfold Sub; eauto.
    now apply topsig_gives_toplike.
    destruct H0; inversion H0; subst; try now inversion H5.
    apply TLAnd.
    apply IHsub1; auto.
    unfold Sub; eauto.
    apply IHsub2; auto.
...
Inductive WFTyp : PTyp -> Prop := 
  ...
  | WFAnd : forall t1 t2, WFTyp t1 -> WFTyp t2 -> OrthoS t1 t2 ->
                     not (TopLike t1) -> not (TopLike t2) ->
                     WFTyp (And t1 t2)
  | WFTop : WFTyp TopT.

Hint Constructors WFTyp Ortho.
...
Lemma not_toplike_and_distr :
  forall t1 t2, ~ TopLike (And t1 t2) ->  ~ TopLike t1 \/ ~ TopLike t2.
Proof.
  intros t1 t2 HTL.
  unfold not in HTL.
  apply Decidable.not_and.
  unfold Decidable.decidable.
  admit. (* assert decidability of TopLike types *)
...
Proof.
  ...
  - destruct IHt1.
    destruct IHt2.
    left; auto.
    right; unfold not; intros HInv; inversion HInv; subst; contradiction.
    right; unfold not; intros HInv; inversion HInv; subst; contradiction.
  - left; apply TLTop.
Qed.   

Lemma toplike_wf_gives_topsig :
  forall t, TopLike t ->
       WFTyp t ->
       TopSig t.
Proof.
  ...
  induction HWFt; auto.
  - inversion HTLt.
  - inversion HTLt; subst; auto.
  - inversion HTLt; subst; contradiction.
Qed.
...
- inversion H4; subst.
  ...
  apply IHhas_type_source_alg2 in H8.
...
(* Case Merge *)
- inversion H4; subst.
  apply IHhas_type_source_alg1 in H9.
  apply IHhas_type_source_alg2 in H11.
