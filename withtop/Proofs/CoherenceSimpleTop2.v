...
Lemma OrthoSNotEq : forall A B, OrthoS A B -> not (TopLike A /\ TopLike B) -> not (A = B). 
intros. unfold OrthoS in H. unfold not; intros. rewrite <- H1 in H, H0.
assert (TopLike A). apply H. apply reflex. apply reflex. destruct H0. auto.
Defined.
...
unfold OrthoS.
intros.
induction C0. inversion H1. inversion H3; subst.
apply H. unfold Sub. exists c. auto. auto.
apply H0. unfold Sub. exists c. auto. auto.
inversion H1. inversion H3; subst.
apply H. unfold Sub. exists c. auto. auto.
apply H0. unfold Sub. exists c. auto. auto.
pose (invAndS1 _ _ _ H1). destruct a.
pose (invAndS1 _ _ _ H2). destruct a.
apply TLAnd. apply IHC0_1; auto. apply IHC0_2; auto.
apply TLTop.
Defined.
...
auto. apply sand2.
auto. apply IHWFTyp2.
unfold OrthoS. intros. apply H2. auto. apply sand3. auto.
apply OTop1.
(* Case Fun t1 t2 *)
induction H.
apply OFunInt. 
...
apply OAnd2.
apply IHWFTyp1. unfold OrthoS. intros.
apply H0. auto. apply sand2. auto.
apply IHWFTyp2. unfold OrthoS. intros.
apply H0. auto. apply sand3. auto.
(* Case t11 -> t12 _|_ T *)
apply OTop1.
(* Case (t11 & t12) _|_ t2 *)
apply OAnd1.
...
Lemma sub_top : forall A B, Sub A B -> TopLike A -> TopLike B.
  intros A B s. destruct s. induction H; intros. auto.
  inversion H1. apply TLAnd. apply IHsub1. auto. apply IHsub2. auto.
  inversion H1; subst. apply IHsub. auto.
  inversion H1; subst. apply IHsub. auto. apply TLTop.
Defined.

Lemma sub_not_top : forall A B, Sub A B -> not (TopLike B) -> not (TopLike A).
  intros. unfold not; intros. apply H0. apply sub_top with (A := A). auto. auto.
Defined.

Lemma uniquesub : forall A B C, 
  OrthoS A B -> Sub (And A B) C -> not (TopLike C) ->  not (Sub A C /\ Sub B C).
Proof.
  intros. unfold OrthoS in H. unfold not. intros. destruct H2. apply H1.
  apply H.  unfold not.
  auto. auto.
Defined.
...
(* Top cases *)
unfold OrthoS. intros.
induction C; try (inversion H0; inversion H1); subst.
apply TLAnd.
apply IHC1.
apply invAndS1 in H. destruct H.
auto.
...
apply TLTop.
unfold OrthoS. intros.
induction C; try (inversion H; inversion H1); subst.
apply TLAnd.
apply IHC1.
apply invAndS1 in H. destruct H.
auto.
