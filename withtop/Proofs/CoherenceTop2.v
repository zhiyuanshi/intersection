...
Inductive TopLikeExt : PTyp -> Prop :=
  | TLTopExt : TopLikeExt TopT
  | TLAndExt : forall A B, TopLikeExt A -> TopLikeExt B -> TopLikeExt (And A B)
  | TLFunExt : forall A B, TopLikeExt B -> TopLikeExt (Fun A B).  
...
Definition OrthoS (A B : PTyp) := 
  (not (TopLike A) /\ not (TopLike B)) /\ (forall C, Sub A C -> Sub B C -> TopLikeExt C).

Lemma applyOrthoS : forall {A B}, OrthoS A B -> forall C, Sub A C -> Sub B C -> TopLikeExt C.
...
Lemma topLike : forall A, TopLike A -> TopLikeExt A.
Proof.
  intros.
  induction H.
  apply TLTopExt.
...
Lemma topLikeWF : forall A, TopLike A -> WFTyp A -> A = TopT.
Proof.
  intros.
  induction H0.
  inversion H.
...
Proof.
  ...
  unfold not; intros.  
  rewrite <- H2 in H0.
  clear H1; clear H2; clear B.
  assert (TopLikeExt A) by (apply H0; apply reflex).
  induction H3.
  apply H.
  apply TLTop.
  contradiction. Print TopLikeExt.
  apply IHTopLikeExt.
  ...
  assert (TopLike A). admit. apply reflex. apply reflex.
contradiction.
Defined.
 *)
...
apply OAnd2.
apply IHWFTyp1.
unfold OrthoS; repeat split.
unfold not; intros HInv; inversion HInv.
assumption.
intros; inversion H2; inversion H7; apply H8; auto.
apply IHWFTyp2.
unfold OrthoS; repeat split.
unfold not; intros HInv; inversion HInv.
assumption.
...
Lemma uniquesub : forall A B C, 
  OrthoS A B -> Sub (And A B) C -> not (TopLikeExt C) ->  not (Sub A C /\ Sub B C).
Proof.
...
Lemma superTopIsToplikeExt :  forall t c,
                                sub TopT t c -> TopLikeExt t.
Proof.
  intros.
  assert (Sub TopT t0).
  unfold Sub.
  ...
  inversion H0.
  inversion H.
  unfold Sub; now exists c2.
  apply TLTopExt.
Qed.
  
Lemma orthoFunTop : forall t1 t2 t3 t4, t2 = TopT -> OrthoS (Fun t1 t2) (Fun t3 t4).
Proof.
unfold OrthoS.
repeat split.
unfold not; intros HInv; inversion HInv.
unfold not; intros HInv; inversion HInv.
subst; intros c H1 H2.
inversion H1.
clear H2.
clear H.
induction c.
...
apply TLAndExt. apply IHC1. unfold Sub. now exists c1.
unfold Sub. now exists c0.
apply IHC2. unfold Sub. now exists c2.
unfold Sub. now exists c3.
(* TopT *)
apply TLTopExt.
(* Case IntFun *)
unfold OrthoS. split.
...
apply TLTopExt.
(* FunFun *)
unfold OrthoS. split. split.
unfold not; intros HInv; inversion HInv.
unfold not; intros HInv; inversion HInv.
intros.
induction C. inversion H2. inversion H4.
apply TLFunExt. inversion IHOrtho. apply H5. inversion H2. inversion H6.
unfold Sub. exists c2. auto.
inversion H3. inversion H6.
unfold Sub. exists c2. auto.
apply TLAndExt. apply IHC1. inversion H2. inversion H4.
unfold Sub. exists c1. auto.
inversion H3. inversion H4.
unfold Sub. exists c1. auto.
apply IHC2. inversion H2. inversion H4.
unfold Sub. exists c2. auto.
inversion H3. inversion H4.
unfold Sub. exists c2. auto. 
apply TLTopExt.
(* FunFun Top (left) *)
apply orthoFunTop; auto.
(* FunFun Top (right) *)
apply ortho_sym.
apply orthoFunTop; auto.
Qed.
...
Lemma same_coercion' : forall B, TopLikeExt B -> WFTyp B -> forall A e1 e2,
  and_coercion (Fun A B) e1 = and_coercion (Fun A B) e2.
intro. intro.
induction H; intros.
(* Case TopT *)
...
(* Case And *)
(*
simpl in IHTopLikeExt1.
simpl in IHTopLikeExt2.
simpl. inversion H1. destruct H6. destruct H6. contradiction.
 *)
admit.
(* Case Fun *)
simpl in IHTopLikeExt.
simpl. inversion H0.
rewrite IHTopLikeExt with (e2 := e2); auto.
Admitted.
...
- inversion H3; inversion H. 
  ...
  destruct H14.
  assert (TopLikeExt t0). apply H15. unfold Sub.
...
- inversion H3; inversion H.
  ...
  destruct H14.
  inversion H14.
  assert (TopLikeExt t0). apply H15.
