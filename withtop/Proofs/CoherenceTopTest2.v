...
Inductive Ortho : PTyp -> PTyp -> Prop :=
  ...
  | OTop1 : forall t1 t2, ~ TopLike t1 -> TopLike t2 -> Ortho t1 t2
  | OTop2 : forall t1 t2, ~ TopLike t2 -> TopLike t1 -> Ortho t1 t2.
...
Proof.
unfold OrthoS. 
intros; destruct H; split; auto. destruct H; auto.
Defined.
...
destruct H. destruct H0. destruct H; destruct H0; auto.
left; unfold not; intro HInv; inversion HInv; auto.
destruct H; destruct H0.
intros.
Require Import Coq.Program.Equality.
destruct H3; dependent induction H3.
apply TLAnd.
apply IHsub1; auto.
destruct H4.
inversion H3; subst; unfold Sub; eauto.
inversion H5.
inversion H5.
apply IHsub2; auto.
destruct H4.
...
apply H1; unfold Sub; eauto.
apply H2; unfold Sub; eauto.
auto.
Defined.
...
Proof.
  ...
  - generalize H0. clear H0. induction H; intros.
    ...
    destruct H2; intros; apply H4; auto.
    apply IHWFTyp2. unfold OrthoS; split; auto.
    left; intros HInv; inversion HInv.
    destruct H2; intros; apply H4; auto.
    apply OAnd2.
    apply IHWFTyp1. unfold OrthoS; split; auto.
    left; intros HInv; inversion HInv.
  ...
  - induction H.
    ...
    unfold OrthoS; split.
    destruct H0; apply Classical_Prop.not_and_or;
    unfold not; intros HInv; inversion HInv; apply H0; now apply TLFun.
    intros. 
    assert (TopLike (Fun (And t1 t0) C)).
    apply H2. apply sfun. apply sand2. apply reflex.
    auto. apply sfun. apply sand3. apply reflex. auto.
    now inversion H5.
    (* Case t11 -> t12 _|_ t21 & t22 *)
    destruct H2.
    destruct (toplike_dec (Fun t1 t2)).
    assert (not (TopLike (And t0 t3))).
    unfold not; intros HInv; inversion HInv; subst.
  ...
  - destruct H. destruct H1.
    ...
    apply OTop1; auto.
    unfold not; intros; destruct H; apply H; now inversion H4.
    apply OAnd1; auto.
    apply IHwft1_1; auto; unfold OrthoS; auto.
    apply IHwft1_2; auto; unfold OrthoS; auto.
  (* Case T _|_ t2 *)
  - destruct H0. destruct H0.
    exfalso; apply H0; apply TLTop.
    apply OTop2; auto.
Qed.
...
Proof.
  ...
  (* Case IntFun *)
  - unfold OrthoS; split; auto.
    left; unfold not; intros HInv; inversion HInv.
  ...
  - destruct IHOrtho. destruct H0. 
    unfold OrthoS. split.
    left; unfold not; intros HInv; apply H0; now inversion HInv.
    intros.
    induction C. inversion H3. inversion H4.
    apply TLFun.
    apply H1; inversion H2; inversion H4; inversion H3; inversion H12;
    unfold Sub; eauto.
    apply TLAnd.
    apply IHC1; inversion H2; inversion H4; inversion H3; inversion H11;
    unfold Sub; eauto.
    apply IHC2; inversion H2; inversion H4; inversion H3; inversion H11;
    unfold Sub; eauto.
    apply TLTop.
    unfold OrthoS. split.
    right; unfold not; intros HInv; apply H0; now inversion HInv.
    intros.
    induction C. inversion H3. inversion H4.
    apply TLFun.
    apply H1; inversion H2; inversion H4; inversion H3; inversion H12;
    ...
    unfold Sub; eauto.
    apply TLTop.   
  - unfold OrthoS; split; auto; intros.
    apply toplike_sub_is_toplike with (A := t2); auto.
  - unfold OrthoS; split; auto; intros.
    apply toplike_sub_is_toplike with (A := t1); auto.
Qed.
...
simpl in IHTopLike1.
simpl in IHTopLike2.
simpl. inversion H1; subst. destruct H6. destruct H2; contradiction.
(* Case Fun *)
simpl in IHTopLike.
simpl. inversion H0.
