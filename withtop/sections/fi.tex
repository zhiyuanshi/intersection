...
and a merge operator.  This calculus is inspired by Dunfield's
calculus~\cite{dunfield2014elaborating}.  However our calculus does
not consider union types, and due to the fact that we are interested
only in disjoint intersections, it also has a few less typing rules
than in Dunfield's calculus.\bruno{rephrase next} The differences are
...
function to have an intersection type.  Sections~\ref{sec:disjoint}
...
\inter B$) and the top type ($\top$) at
the type-level, and merges ($e_1 \mergeop e_2$) and the top value ($\top$) the at the term level.
...
$B$. We also include $\top$ and integer types $\code{Int}$.
...
%\end{comment}
  ...
  \begin{mathpar}
    \formsub \\
    \rulesubint \and \rulesubtop \and \rulesubfun \and \rulesubinter 
    \and \rulesubinterldis \and \rulesubinterrdis 
  \end{mathpar}

  \begin{mathpar}
    \formwf \\
    \rulewfint \and \rulewffun \and \rulewfinterdis \and \rulewftop
  \end{mathpar}
...
{\renewcommand{\yields}[1]{}
  \begin{mathpar}
    ...
    \ruletmergedis \and \ruletsub \and \rulettop
  \end{mathpar}
}
...
\paragraph{Differences to Dunfield's Calculus}
