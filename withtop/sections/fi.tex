...
\paragraph{More differences to Dunfield's calculus}
...
type-system does not have those rules. One reason is that such rules
make less sense in a system with disjoint intersions. For example, 
Dunfield's calculus includes the following typing rule:

\bruno{rule and discussion missing here!}

Another difference is that, in Dunfield's language, subtyping produces
coercions that are source language terms.
