...
scripts.\bruno{should we mention an extended version of the paper?}
...
\reflabel{\labelsubinter}. With the ordinary constraint, one can guarantee that at
...
extended with annotation expressions $e : A$. \bruno{References to
  bidirectional type-checking needed somewhere?}
...
\begin{theorem}[Type safety]
  ...
  value $v$.
\end{theorem}\bruno{Joao, can you please adjust the lemmas for the
...
modulo some additional type-annotations.\bruno{Need to add the
  definition of erasure here}
...
bidirectional type-checking in Figure~\ref{} is that, given an expression $e$, if it is
...
\begin{proof}
  ...
  unique.\bruno{This explanation is no longer valid; we need to talk
    about the subsumption rule instead (and still about the Lemma 5);
    and we need to talk about the role of uniqueness of type-inference.}
