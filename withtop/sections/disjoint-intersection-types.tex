...
\begin{restatable}[Unique elaboration]{theorem}{uniqueelaboration}
  ...
  \begin{itemize}
    ...
    \item If $\jcheck \Gamma e {A_1} \yields {E_1}$ and $\jcheck \Gamma e {A_2} \yields
          {E_2}$, then $E_1 \equiv E_2$.
  \end{itemize}
  (``$\equiv$'' means syntactical equality, up to
  $\alpha$-equality.)
...
\begin{proof}
  ...
  that $C$ is also unique, by
  Lemma~\ref{lemma:unique-coercion}.\bruno{Still not good: what is C
    and what is A? They are not defined above.}
  The latter requires proving uniqueness of $A_1$.
  We can show this, again, by Theorem~\ref{theorem:uniqueness-type-inference} and consequently use 
