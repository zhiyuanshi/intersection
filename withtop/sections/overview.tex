...
In other words the results of the program are incoherent.\bruno{rephrase better?}
...
\subsection{The Challenges of Disjointness Intersection Types}
Informally saying that two types are disjoint means that the set of 
values of both types are disjoint. Disjointness is potentially useful 
for coherence, since it rules out ambiguity when looking up a value 
of a certain type in an intersection. However there are several issues
that need to be addressed first in order to design a calculus with disjoint 
intersection types and that ensures coherence. 

\paragraph{Disjointness of non-atomic types} 
A first issue is what
does disjointness mean for non-atomic types?  For atomic types such
as \lstinline$Int$ and \lstinline$Bool$, it is easy to see that
disjointness holds when the two types are different. Thus the following 
intersections would be disjoint

\noindent whereas any intersection with equal atomic types are not disjoint.

\paragraph{What is a suitable formal definition of
  disjointness?}
...
by requiring the two types not to share a common supertype - excluding any type of
the form ${A_k} \to \top$. 
