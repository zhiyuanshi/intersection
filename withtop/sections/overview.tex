...
\begin{definition}[Simple disjointness]\label{def:simple_dis}
  Given two types $A$ and $B$, those types are disjoint
...
\subsection{Disjointness Intersection Types with $\top$} 
...
top-like types also allow for other types which are \emph{syntactically different}
...
\noindent The presence of  \reflabel{\labelsubinter} means that both  
...
subtype of the merge, the type of the program is $\top$, and its
result is always the \emph{same} $\top$ value. Secondly,
...
types, as well as proper top types. \bruno{finish this}
