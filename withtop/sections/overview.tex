...
form or another\cite{CDuce}.\bruno{Reviewer asked for more references here.}
...
formulation, where intersection types are introduced by a \emph{merge
  operator}.\bruno{Perhaps mention other works with the merge operator, 
including lambda\&} 
...
A first attempt at a definition for disjointness is to require that, given two types $A$ and $B$, both types are not
...
\begin{definition}[Disjointness with $\top$]
  Given two types $A$ and $B$, two types are disjoint
...
In this definition the idea is to allow $A$ and $B$ to share some 
...
incoherence.\bruno{Explanation probably needs to be expanded}
...
%- excluding any type of the form ${A_k} \to \top$.
...
\paragraph {Is disjointness sufficient to ensure coherence?} A final question
