...
\FI is an thin extension on top of \F that is elaborated into \Fend, a variant of
System F. System F, or polymorphic lambda calculus lays the foundation of
...
As a running example, let's consider elaborating a term
...
There has been recently a suprisingly lightweight solution to the expression
problem that takes advantage of covariant return types in Java. We show that FI
is able to solve the expression problem in the same spirit. The

Encoding OAs
Mixins

\lstinputlisting[linerange=2-5]{../src/Examples.sf} % APPLY:linerange=MIXIN_LIB
