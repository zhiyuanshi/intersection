...
If
  $\istype \Gamma {\for {\alpha \disjoint B} C}$,
  $\istype \Gamma A$,
  $\isdisjoint \Gamma A B$,
then
  $\istype \Gamma {\subst A \alpha C}$.
\end{lemma}
Instantiating a ``forall'' type results in a well-formed type if the type provided satisfies the disjoint constraint.
\begin{proof}
Since
\begin{mathpar}
  \istype \Gamma {\for {\alpha \disjoint B} C}
\end{mathpar}
by \rulelabel{WFForall} we have
\begin{mathpar}
  \istype \Gamma B \\ \istype {\Gamma, \alpha \disjoint B} C
\end{mathpar}
\end{proof}
