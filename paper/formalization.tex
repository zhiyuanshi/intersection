...
A naive substitution can violate the disjoint constraint in the context. For example (assuming the existence of some base type $\tyint$),
\begin{mathpar}
\inferrule* [right=Foo]
  {\alpha \disjoint \tyint \in {\alpha \disjoint \tyint}}
  {\istype {\alpha \disjoint \tyint} \alpha}
\end{mathpar}
is provable by \ldots. But after the substitution of $\tyint$ for $\alpha$, \ldots
\begin{mathpar}
\inferrule
  {}
  {\istype {\alpha \disjoint \tyint} {\subst \tyint \alpha \alpha}}
\end{mathpar}

\begin{lemma}
If $\isdisjoint \Gamma A B$ and the $R$, $\gamma$ pair does not violate any of the disjointness constraints in $\Gamma$, then $\isdisjoint \Gamma {\subst R \gamma A} {\subst R \gamma B}$.
\end{lemma}

\begin{lemma}{Substitution lemma} \label{substitution-lemma}
If $\istype \Gamma R$, $\istype \Gamma S$, and the $R$, $\gamma$ pair does not violate any of the disjointness constraints in $\Gamma$\todo{Elaborate}, then $\istype \Gamma {\subst R \beta S}$.
\end{lemma}
...
\begin{itemize}
  ...
  \begin{mathpar}
    \wffun
  \end{mathpar}
  By i.h., $\istype \Gamma {\subst R \gamma A}$ and $\istype \Gamma {\subst R \gamma B}$. By the def. of substitution, $\istype \Gamma {\subst R \gamma {A \to B}}$.

  \item Case
  \begin{mathpar}
    \wfforall
  \end{mathpar}
  By i.h., $\istype \Gamma {\subst R \gamma A}$ and $\istype {\Gamma, \alpha \disjoint A} {\subst R \gamma B}$. By the def. of substitution, $\istype \Gamma {\subst R \gamma {\for {\alpha \disjoint A} B}}$~\todo{How about $A$?}.

  \item Case
  \begin{mathpar}
    \wfinter
  \end{mathpar}
  By i.h., $\istype \Gamma {\subst R \gamma A}$ and $\istype \Gamma {\subst R \gamma B}$. By some magic lemma~\todo{TODO}, we also have $\isdisjoint \Gamma {\subst R \gamma A} {\subst R \gamma B}$. Therefore by \rulelabel{WFInter}, $\istype \Gamma {\subst R \gamma {A \intersect B}}$.
\end{itemize}
\end{proof}
...
The special case to consider is \rulelabel{TyTApp}. $\subst A \alpha B$ is also well-formed by \cref{substitution-lemma}.
\end{proof}
