...
If $\isdisjoint \Gamma A B$ and the $\isdisjoint \Gamma \beta R$, then $\isdisjoint \Gamma {\subst R \beta A} {\subst R \beta B}$.
\end{lemma}
...
If $\istype \Gamma R$, $\istype \Gamma S$, and $\isdisjoint \Gamma \beta R$, then $\istype \Gamma {\subst R \beta S}$.
\end{lemma}
...
\begin{itemize}
  ...
  \begin{mathpar}
    \wfinter
  \end{mathpar}
  By i.h., $\istype \Gamma {\subst R \beta A}$ and $\istype \Gamma {\subst R \beta B}$. By some magic lemma~\todo{TODO}, we also have $\isdisjoint \Gamma {\subst R \beta A} {\subst R \beta B}$. Therefore by \rulelabel{WFInter}, $\istype \Gamma {\subst R \beta {A \intersect B}}$.
\end{itemize}
\end{proof}
...
\end{mathpar}
Next we do a case analysis on the shape of $C$. The interesting case is when $C$ is an intersection type.
\begin{mathpar}
  \wfinter
\end{mathpar}
Let $C = C_1 \intersect C_2$. We know $\istype \Gamma {C_1 \intersect C_2}$ and need to show
\[ \istype \Gamma {\subst A \alpha {C_1 \intersect C_2}} \]
which is just
\[ \istype \Gamma {\subst A \alpha {C_1} \intersect \subst A \alpha {C_2}} \]
by the definition of the substitution function.
\end{proof}
