...
If $\isdisjoint \Gamma A B$ and the $\isdisjoint \Gamma \gamma R$, then $\isdisjoint \Gamma {\subst R \gamma A} {\subst R \gamma B}$.
\end{lemma}

\begin{lemma}{Substitution} \label{substitution-lemma}
If $\istype \Gamma R$, $\istype \Gamma S$, and $\isdisjoint \Gamma \gamma R$, then $\istype \Gamma {\subst R \gamma S}$.
\end{lemma}
...
\begin{proof}
By induction on the derivation of $\hastype \Gamma e A$. The case of \rulelabel{TyTApp} needs special attention since we need to show that the result of substitution ($\subst A \alpha C$) is well-formed.
\begin{mathpar}
  \tytapp
\end{mathpar}
\end{proof}
