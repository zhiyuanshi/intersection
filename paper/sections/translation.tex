...
labels by \rulename{RcdIntro}. Hence $ \{ eval = 4 \} ,, \{ print = ``4'' \} $
...
\item{\bf Translation}
  ...
  \rulename{Merge} translates merges into pairs. \rulename{RcdIntro} uses the
  same System $ F $ expression $ E $ for $ e $ as for $ \recordintro l e $. And in
  \rulename{RcdEim} and \rulename{RcdUpd} the coercions generated by the ``get''
  and ``put'' rules will be used to coece the main \name expression.

  \rulename{RcdElim} typechecks $ e $ and use the ``get'' rule to return the
  type of the field $ \ty_1 $ and the coecion $ C $. The type of the whole
  expression is $ \ty_1 $ and its translation of $ \app C E $.

  \rulename{RcdUpd} is similar to \rulename{RcdElim} in that it uses the
...
\item{\bf ``put'' rules}\bruno{Missing example (and derivation)}
