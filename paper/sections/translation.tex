...
to $ \ty_3 $, then we can conclude that $ \ty_1 \Intersect \ty_2 $ is also a subtype of
$ \ty_3 $ and the new coercion is a function that takes a value $ x $ of type
$ \ty_1 \Intersect \ty_2 $, project $ x $ on the first item, and apply $ C $ to it.
...
\paragraph{``get'' Rules}
  ...
  function specialized to type $ \image {\RecTy l \ty} $ \rulename{GetLeft} and
