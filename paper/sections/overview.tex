...
A number of OO languages, such as Java, C\#, Scala, and Ceylon\george{cite?}, already
...
% Dunfield's approach is the $\Merge$ operator, which allows for run-time
...
% interpretation of types: $ A \Intersect B $ stands for the intersection of
...
% $ A \Intersect B $ satisfies both of the interfaces of $ A $ and $ B $. For
...
\bruno{we support ``object/value-level composition''. Contrast with 
class-level or trait level composition in Scala.}
...
shall see in the next section, in solving modularity problems. The most simple
example is a function that just uses the merge operator:
% \bruno{use the join example here.}
\begin{lstlisting}
let merge [A] [B] (x: A) (y: B) : A & B = x ,, y
\end{lstlisting}
The key novel feature is that in our language we allow the intersection of type
parameters.
