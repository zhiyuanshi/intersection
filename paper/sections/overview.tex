...
Consider defining a type $G$ by adding fields to an existing type $F$
in Cook's notation~\cite{cook1989inheritance}:
\[
G = F + \{ l_1:\tau_1, \ldots, l_n:\tau_n \}
\]
where the plus sign means a field in the increment will override an existing
field in $F$. ``Inheritance is not subtyping'' means that structually, $G$ can
...
$G$ have no relation. However, most OO languages conflate the two ideas, leading
to cases where the type system gets in the way of expressing semantically
correct programs. This problem is exactly what has made using the visitor
pattern in many OO languages hard to extend.

Now let's look at a concrete example. An initial program consists of the two
types:
\begin{lstlisting}
type A = {x1: Int};
type F = {f: A -> Int};
...
$F$ is through \lstinline$G extends F$. Unfortunately, $G$ has become a subtype
of $F$.
