...
Consider defining $G$ by adding fields to an existing type $F$
\[
...
$G$ can be: 
(1) a subtype of $F$, 
(2) a supertype of $F$, or 
(3) $G$ and $F$ can have no relation at all! 
...
Conceivably, mixing the two cases above will lead to the third possibility.
...
This problem is exactly what has made using the visitor pattern in many OO languages hard to extend.
...
record operations can be defined in terms of restriction. For example, we can define record
