...
$ \Forall \alpha \ty $. The last two are novel. $ \ty_1 \Intersect \ty_1 $ is
...
expressions $ \Lam x \ty e $ abstract expression $ e $ over values of
...
$ \ty_1 \Intersect \ty_2 $. $ \RecCon l e $ constructs a single-field record.
...
\begin{enumerate}
\item $ \ty_1 \Intersect \ty_2 $ is a subtype of $ \ty_3 $, if \emph{either} $ \ty_1 $ or
  $ \ty_2 $ are subtypes of $ \ty_3 $,

\item $ \ty_1 $ is a subtype of $ \ty_2 \Intersect \ty_3 $, if $ \ty_1 $ is a subtype of
  both $ \ty_2 $ and $ \ty_3 $.
...
%   \fields{\ty_1 \Intersect \ty_2} & = & \fields{\ty_1} \dplus \fields{\ty_2} \\
%   \fields{\RecTy l \ty} & = & \rel l t
% \end{array} \]
